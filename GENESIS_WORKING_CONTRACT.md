...
- the separate bounded runtime/backtest follow-up packet for that surface is now recorded at `docs/governance/ri_policy_router_aged_weak_plus_stability_interaction_runtime_packet_2026-04-27.md`; it has now been pre-code reviewed `APPROVED_WITH_NOTES`, keeps scope inside `src/core/strategy/ri_policy_router.py` plus focused router/scenario tests on the exact fail-B subject (`tBTCUSD`, `3h`, `2023-12-01 -> 2023-12-31`, warmup `120`, `curated_only`, canonical env), and treats the problem strictly as an aged-weak plus same-origin stability/min-dwell interaction rather than a generic stability weakening or another raw second-hit retry; implementation must stay behind the existing `research_policy_router.enabled` gate, must not hardcode timestamps/symbols/paths, must clear any one-shot provenance state fail-closed, and must not create a new runtime/public debug contract merely to prove the slice
...
6. the low-zone family and aged-weak follow-up are now both resolved honestly for this chain: `docs/governance/ri_policy_router_low_zone_bars8_evidence_floor_runtime_packet_2026-04-27.md` is closed negative / reverted, `docs/governance/ri_policy_router_bars7_continuation_persistence_runtime_packet_2026-04-27.md` remains the last retained positive runtime slice with exact helper-hit proof `results/backtests/ri_policy_router_bars7_continuation_20260427/fail_b_helper_hit_timestamps.json = ["2023-12-20T03:00:00+00:00"]`, and `docs/governance/ri_policy_router_aged_weak_second_hit_release_runtime_closeout_2026-04-27.md` now closes the separate aged-weak continuation residual surface negative because unchanged stability controls keep the second-hit rows in `RI_no_trade_policy`; any reopen from here requires a fresh packet that explicitly addresses the aged-weak-plus-stability interaction question rather than another router-local second-hit assumption
