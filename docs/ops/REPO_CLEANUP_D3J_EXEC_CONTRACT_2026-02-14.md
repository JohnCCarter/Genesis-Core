# Repo Cleanup D3J Execution Contract (2026-02-14)
...
- `scripts/diagnose_fib_flow.py` (move)
- `scripts/archive/debug/2026-02-14/diagnose_fib_flow.py` (new path)
...
- Move-only: inga innehållsändringar i `scripts/diagnose_fib_flow.py`.
- Inga runtime/API/config-ändringar.
- Befintliga docs-referenser (aktiva + historiska) uppdateras inte i D3J; påverkan dokumenteras i rapporten.
...
2. D3J kontrakt + exekveringsrapport dokumenterade.
...
- D3J i detta kontrakt är kandidatvis destruktiv flytt av ett diagnose-script med måttlig referensyta.
