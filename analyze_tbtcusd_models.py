...
def analyze_tbtcusd_models():
    ...
    for model_file in tbtcusd_models:
        try:
            ...
            if "_" in model_name and not any(x in model_name for x in ["_v", "_metrics", "_holdout", "_provenance"]):
...
def print_analysis_report(current_models, outdated_models, metrics_models, other_models):
    ...
    for model in current_models:
        print(f"  OK {model['file']} - {model['schema_len']} features - {model['size']} bytes - {model['modified'].strftime('%Y-%m-%d %H:%M')}")
    
    print(f"\nFÖRÅLDRADE MODELLER ({len(outdated_models)}):")
    for model in outdated_models:
        print(f"  OLD {model['file']} - {model['reason']} - {model['size']} bytes - {model['modified'].strftime('%Y-%m-%d %H:%M')}")
    
    print(f"\nMETRICS MODELLER ({len(metrics_models)}):")
    for model in metrics_models:
        print(f"  METRICS {model['file']} - {model['size']} bytes - {model['modified'].strftime('%Y-%m-%d %H:%M')}")
    
    print(f"\nANDRA MODELLER ({len(other_models)}):")
    for model in other_models:
        print(f"  OTHER {model['file']} - {model['size']} bytes - {model['modified'].strftime('%Y-%m-%d %H:%M')}")
