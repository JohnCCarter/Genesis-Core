# Feature Attribution v1 — execution slice 2 packet

Date: 2026-03-31
Branch: `feature/ri-role-map-implementation-2026-03-24`
Status: `historical Feature Attribution v1 execution slice 2 packet / retained for provenance / not current branch guidance`
...
- **Required Path:** `Full`
- **Objective:** Execute the second governed Feature Attribution v1 identification slice for `Hysteresis gate seam` only and classify it descriptively as `additive`, `neutral`, `adverse`, `inconclusive`, or `invalid` against the locked baseline.
- **Candidate:** `Hysteresis gate seam`
- **Base SHA:** `68537da2`

### Scope

- **Scope IN:** this execution packet; one explicit single-row execution slice for `Hysteresis gate seam` only; locked baseline provenance from `results/research/fib_baseline_backtest_v1/summary.md` and `results/research/fib_baseline_backtest_v1/metrics.json`; existing internal execution path only (`src/core/pipeline.py`, `src/core/backtest/engine.py`, `src/core/strategy/evaluate.py`, `src/core/strategy/decision.py`) with no code changes during execution; one human-readable report under `results/research/feature_attribution_v1/reports/`; one machine-readable manifest under `results/research/feature_attribution_v1/manifests/`.
- **Scope OUT:** no source-code changes; no tests added/edited; no new CLI surface; no new env-var surface; no runner expansion; no execution of any row other than `Hysteresis gate seam`; no cluster execution; no multi-row ranking; no promotion/cutover semantics; no edits under `config/strategy/champions/**`; no fib reopening; no artifacts outside the explicitly named report/manifest outputs.
- **Expected changed files:** `docs/decisions/feature_attribution/v1/feature_attribution_v1_exec_slice2_hysteresis_identification_packet_2026-03-31.md`, `results/research/feature_attribution_v1/reports/fa_v1_hysteresis_20260331_01.md`, `results/research/feature_attribution_v1/manifests/fa_v1_hysteresis_20260331_01.json`
- **Max files touched:** `3`
...
| Field                   | Value                                                     |
| ----------------------- | --------------------------------------------------------- |
| `run_id`                | `fa_v1_hysteresis_20260331_01`                            |
...
- `policy["feature_attribution"]["selected_row_label"] = "Hysteresis gate seam"`
...
- repo-local acknowledgment that `.github/skills/feature_parity_check.json` remains conditional and non-executable for this slice because no feature-computation surface is in scope
- manual provenance check that baseline summary + metrics + config source align with the frozen execution inputs above
- manual effective-config check that the only intentional candidate diff is neutralization of `gates.hysteresis_steps`
...
- any gate failure
- any evidence that the effective-config diff escapes `gates.hysteresis_steps`
- any attempt to rank multiple rows or turn the descriptive label into a direct action

### Output required

- one executed single-row candidate-vs-baseline comparison for `Hysteresis gate seam`
- one descriptive classification using the locked Phase 5 vocabulary
- one report at `results/research/feature_attribution_v1/reports/fa_v1_hysteresis_20260331_01.md`
- one manifest at `results/research/feature_attribution_v1/manifests/fa_v1_hysteresis_20260331_01.json`
...
- `candidate_effective_hysteresis_steps`
- `baseline_effective_hysteresis_steps`
...
This packet authorizes only the second identification slice.
