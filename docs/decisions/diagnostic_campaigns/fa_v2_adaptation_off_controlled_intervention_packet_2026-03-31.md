...
- **Scope IN:** `docs/decisions/diagnostic_campaigns/fa_v2_adaptation_off_controlled_intervention_packet_2026-03-31.md`; `src/core/strategy/decision_gates.py`; targeted tests proving default behavior is unchanged when the switch is absent and that base thresholds become authoritative when the switch is explicitly disabled; deterministic canonical backtest execution using `GenesisPipeline.setup_environment(seed=42)` + `GenesisPipeline.create_engine(...)` + `BacktestEngine.run(...)`; effective-config snapshot and diff proof for every governed run; new research artifacts under `results/research/fa_v2_adaptation_off/` only.
