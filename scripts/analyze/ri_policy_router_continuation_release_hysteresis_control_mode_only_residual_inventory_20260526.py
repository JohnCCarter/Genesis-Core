...
SUBJECT_IDS = ("2021-04", "2023-05")
DORMANT_FIELD_DECAY_RELATIVE = Path(
    "results/evaluation/ri_policy_router_continuation_release_hysteresis_candidate_dormant_field_decay_2026-05-26.json"
)
OUTPUT_ROOT_RELATIVE = Path("results/evaluation")
OUTPUT_FILENAME = "ri_policy_router_continuation_release_hysteresis_control_mode_only_residual_inventory_2026-05-26.json"
STATUS_OK = "continuation_release_hysteresis_control_mode_only_residual_inventory_generated"
STATUS_FAIL_CLOSED = (
    "continuation_release_hysteresis_control_mode_only_residual_inventory_fail_closed"
)
PACKET_STATUS_CANDIDATE_ONLY = "candidate_only_control_mode_residual_rows_persist_after_reentry"
...
class ControlModeOnlyResidualInventoryError(RuntimeError):
    pass
...
def _load_action_position_subjects() -> dict[str, dict[str, Any]]:
    payload = local_packet._coerce_dict(
        json.loads(
            (ROOT_DIR / candidate_replay.ACTION_POSITION_EQUIVALENCE_RELATIVE).read_text(
                encoding="utf-8"
            )
        ),
        field_name="control_mode_only_inventory.action_position.payload",
    )
    subject_payloads = local_packet._coerce_dict(
        payload.get("subject_payloads"),
        field_name="control_mode_only_inventory.action_position.subject_payloads",
    )
    subjects: dict[str, dict[str, Any]] = {}
    for subject_id in SUBJECT_IDS:
        subject_payload = local_packet._coerce_dict(
            subject_payloads.get(subject_id),
            field_name=f"control_mode_only_inventory.action_position.subject_payloads.{subject_id}",
        )
        row_analyses = [
            candidate_replay._coerce_episode_row(
                raw_row,
                field_name=(
                    f"control_mode_only_inventory.action_position.subject_payloads.{subject_id}.row[{index}]"
                ),
            )
            for index, raw_row in enumerate(
                local_packet._coerce_list(
                    ...
                    field_name=(
                        f"control_mode_only_inventory.action_position.subject_payloads.{subject_id}.envelope_row_analysis"
                    ),
                )
            )
        ]
...
def _load_candidate_reentry_timestamp() -> str:
    payload = local_packet._coerce_dict(
        json.loads((ROOT_DIR / DORMANT_FIELD_DECAY_RELATIVE).read_text(encoding="utf-8")),
        field_name="control_mode_only_inventory.dormant_field_decay.payload",
    )
    subject_payload = local_packet._coerce_dict(
        payload.get("subject_payload"),
        field_name="control_mode_only_inventory.dormant_field_decay.subject_payload",
    )
    field_decay_matrix = local_packet._coerce_list(
        subject_payload.get("field_decay_matrix"),
        field_name="control_mode_only_inventory.dormant_field_decay.field_decay_matrix",
    )
    reentry_rows = [
        local_packet._coerce_dict(
            row,
            field_name=f"control_mode_only_inventory.dormant_field_decay.field_decay_matrix[{index}]",
        )
        for index, row in enumerate(field_decay_matrix)
        if local_packet._coerce_optional_str(
            local_packet._coerce_dict(
                row,
                field_name=f"control_mode_only_inventory.dormant_field_decay.scan[{index}]",
            ).get("phase")
        )
        == "first_shared_open_after_unlock"
    ]
    if len(reentry_rows) != 1:
        raise ControlModeOnlyResidualInventoryError(
            f"Expected exactly one candidate re-entry row, found {len(reentry_rows)}"
        )
    return local_packet._coerce_str(
        reentry_rows[0].get("timestamp"),
        field_name="control_mode_only_inventory.dormant_field_decay.reentry.timestamp",
    )
...
def _serialize_row(
    row: dict[str, Any], *, candidate_reentry_timestamp: str | None
) -> dict[str, Any]:
    baseline = row["baseline"]
    release_zero = row["release_zero"]
    timestamp = str(row["timestamp"])
    at_or_after_candidate_reentry = False
    if candidate_reentry_timestamp is not None:
        at_or_after_candidate_reentry = candidate_replay._parse_timestamp(
            timestamp
        ) >= candidate_replay._parse_timestamp(candidate_reentry_timestamp)
    return {
        "timestamp": timestamp,
        "classification": str(row["classification"]),
        "at_or_after_candidate_reentry": at_or_after_candidate_reentry,
...
def _build_subject_payload(
    ...
    *,
    candidate_reentry_timestamp: str,
) -> dict[str, Any]:
    ...
    serialized_rows = [
        _serialize_row(
            row,
            candidate_reentry_timestamp=(
                candidate_reentry_timestamp if subject_id == "2021-04" else None
            ),
        )
    ...
    return {
        ...
        "all_rows_at_or_after_candidate_reentry": (
            all(bool(row["at_or_after_candidate_reentry"]) for row in serialized_rows)
            if subject_id == "2021-04" and serialized_rows
            else None
        ),
...
def _build_packet_summary(subject_payloads: dict[str, dict[str, Any]]) -> dict[str, Any]:
    candidate_payload = subject_payloads["2021-04"]
    control_payload = subject_payloads["2023-05"]
    ...
    if candidate_count == 0 and control_count == 0:
        ...
        inference = (
            "Neither the candidate nor the control retains any rows where `switch_control_mode` is the only residual "
            "difference, so the breadcrumb disappears before a dedicated inventory is even needed."
        )
    elif candidate_count > 0 and control_count == 0:
        status = PACKET_STATUS_CANDIDATE_ONLY
        inference = (
            "Only the `2021-04` candidate retains rows where `switch_control_mode` is the sole remaining diff field. "
            "Those rows persist after the first shared re-entry while action, policy selection, switch reason, size, "
            "and execution effect already match."
        )
    else:
    ...
    return {
        ...
        "candidate_all_rows_at_or_after_reentry": candidate_payload.get(
            "all_rows_at_or_after_candidate_reentry"
        ),
        "inference": inference,
        "next_hypothesis": (
            "If this breadcrumb remains the only surviving candidate-specific residual, the next honest decision is "
            "whether to inspect the semantics of `switch_control_mode` directly or retire `2021-04` and widen to the "
            "next negative-like month."
        ),
    }
...
def _build_fail_closed_result(reason: str) -> dict[str, Any]:
    return {
        "audit_version": "ri-policy-router-continuation-release-hysteresis-control-mode-only-residual-inventory-2026-05-26",
        ...
        "inputs": {
            ...
            "candidate_dormant_field_decay_artifact": str(DORMANT_FIELD_DECAY_RELATIVE),
            "working_contract_reference": str(local_packet.WORKING_CONTRACT_REFERENCE),
            "subjects": list(SUBJECT_IDS),
            "focus_surface": "rows where switch_control_mode is the only surviving diff field",
        },
    }


def run_control_mode_only_residual_inventory() -> dict[str, Any]:
    candidate_reentry_timestamp = _load_candidate_reentry_timestamp()
    subject_sources = _load_action_position_subjects()
    subject_payloads = {
        subject_id: _build_subject_payload(
            subject_id,
            subject_sources[subject_id],
            candidate_reentry_timestamp=candidate_reentry_timestamp,
        )
        for subject_id in SUBJECT_IDS
    }
    ...
    return {
        ...
        "classification_boundary": {
            "reference_surface": (
                "artifact-only inventory over the landed action-position-equivalence artifact, limited to rows where "
                "`switch_control_mode` is the sole surviving diff field"
            ),
            "question": (
                "Inside the exact local envelopes, are control-mode-only residual rows unique to the `2021-04` "
                "candidate, or do they also appear in the `2023-05` control?"
            ),
...
def main() -> int:
    ...
    try:
        result = run_control_mode_only_residual_inventory()
    except ControlModeOnlyResidualInventoryError as exc:
