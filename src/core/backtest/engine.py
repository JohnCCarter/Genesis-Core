...
from core.backtest.engine_candle_cache import (
    CandleCache,
    _debug_backtest_enabled,
    _precompute_cache_key_material,
)
from core.backtest.engine_htf_exit_dispatcher import check_htf_exit_conditions
from core.backtest.engine_precompute import compute_precomputed_features
...
class BacktestEngine:
    ...
    def load_data(self) -> bool:
        ...
        if getattr(self, "precompute_features", False):
            self._precomputed_features = compute_precomputed_features(
                candles_df=self.candles_df,
                htf_candles_df=self.htf_candles_df,
                precompute_cache_key_fn=self._precompute_cache_key,
                logger=_LOGGER,
            )
    ...
    def _check_htf_exit_conditions(
        ...
        """Check HTF Fibonacci exit conditions via dispatcher (no behavior change)."""
        return check_htf_exit_conditions(
            self,
