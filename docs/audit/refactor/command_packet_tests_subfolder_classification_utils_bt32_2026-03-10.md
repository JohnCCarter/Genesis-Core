...
- **Required Path:** `Full`
- **Objective:** Relocate `test_triple_barrier.py` till `tests/utils/` med no-behavior-change.
- **Candidate:** `tests/test_triple_barrier.py -> tests/utils/test_triple_barrier.py`
- **Base SHA:** `7877ce46`
- **Category:** `tooling`
...
- **Risk:** `LOW` — why: isolated labeling utility-test utan path/cwd/IO-känslighet
- **Required Path:** `Full`
- **Objective:** Continue utils-category migration med låg-risk-kandidat.
- **Candidate:** `test_triple_barrier.py`
- **Base SHA:** `7877ce46`

### Scope

- **Scope IN:**
  - `tests/test_triple_barrier.py` (move source)
  - `tests/utils/test_triple_barrier.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (BT32-rad)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt32_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_triple_barrier.py`
...
- `repo_clean_refactor` (repo-lokal SPEC-skill; skill-run evidens markeras som pending tills verifierbar artifact-path finns i detta repo)
...
- Candidate file inspected (`tests/test_triple_barrier.py`): inga `__file__`-referenser, ingen relativ fil-I/O eller cwd-beroende.
- `python -m pytest --collect-only -q tests/test_triple_barrier.py` -> `18` tests collected.
- Repo-wide path-reference check: inga externa träffar utanför BT32-packetet för `tests/test_triple_barrier.py`.
- `tests/utils/test_triple_barrier.py` saknas före flytt (ingen target-kollision).
...
- Gates: **PASS**
  ...
  - `python -m pytest -q tests/utils/test_triple_barrier.py` -> PASS (`18 passed`)
...
- Opus post-diff audit: **APPROVED** (Opus 4.6 Governance Reviewer, 2026-03-10; commit/push explicit tillåten)

### Implementation Report (BT32)

- **Scope summary (IN):**
  - Relokerade `tests/test_triple_barrier.py` -> `tests/utils/test_triple_barrier.py` (path-only, oförändrat testinnehåll).
  - Uppdaterade `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` med BT32-rad.
...
- **Scope summary (OUT):**
  ...
  - Ingen ytterligare testflytt utöver BT32-kandidaten.
- **File-level changes:**
  - `tests/test_triple_barrier.py` -> deleted (move source)
  - `tests/utils/test_triple_barrier.py` -> added (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` -> BT32-rad tillagd
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt32_2026-03-10.md` -> packet + evidence uppdaterad
...
- **Artifacts:** BT32 packet + flyttdiff + planuppdatering + gate logs
