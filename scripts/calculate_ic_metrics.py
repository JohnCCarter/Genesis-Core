...
def calculate_forward_returns(close_prices, horizons=[1, 5, 10, 20]):
    """
...
def calculate_rolling_ic(predictions, returns, window=252):
    ...
    valid_indices = [i for i, r in enumerate(returns) if r is not None and not np.isnan(predictions[i])]
    ...
    for i in range(window, len(pred_valid)):
        ic = spearmanr(
            pred_valid[i-window:i],
            ret_valid[i-window:i]
        )[0]
...
def test_model_predictions(model_path, symbol, timeframe, horizons=[10]):
    """
    ...
    print(f"[IC] Calculating IC metrics...")
    results = {}
...
def test_individual_features(symbol, timeframe, horizons=[10]):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate Information Coefficient metrics"
    )
    ...
    parser.add_argument(
        "--test-features",
        action="store_true",
        help="Test individual features instead of model"
    )
    ...
    if args.test_features:
        ...
        if len(feature_ics) > 0:
            ...
            if len(rolling_ic) > 0:
                plot_path = output_dir / f"{args.symbol}_{args.timeframe}_{best_feature}_rolling_ic.png"
                plot_rolling_ic(
                    rolling_ic,
        ...
        for horizon_key, metrics in results.items():
            ...
            print(f"\n  ASSESSMENT:")
            if ic > 0.05:
                print(f"    [+] EXCELLENT IC (> 0.05)")
            elif ic > 0.03:
                print(f"    [+] GOOD IC (> 0.03)")
            elif ic > 0.01:
                print(f"    [!] WEAK IC (> 0.01)")
            else:
                print(f"    [-] VERY WEAK IC (< 0.01)")
            
            if icir > 1.0:
                print(f"    [+] EXCELLENT ICIR (> 1.0)")
            elif icir > 0.5:
                print(f"    [+] GOOD ICIR (> 0.5)")
            else:
                print(f"    [-] POOR ICIR (< 0.5)")
        ...
        if len(rolling_ic) > 0:
            ...
            plot_rolling_ic(
                rolling_ic,
                f"Model Rolling IC ({args.symbol} {args.timeframe})",
                plot_path
            )
