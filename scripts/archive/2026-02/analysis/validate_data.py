...
def check_zero_volume(df: pd.DataFrame) -> dict:
    ...
    examples = []
    for _idx, row in zero_volume.head(10).iterrows():
        examples.append(
            {
                "timestamp": row["timestamp"].isoformat(),
                "close": row["close"],
...
def check_ohlc_logic(df: pd.DataFrame) -> dict:
    ...
    invalid_examples = []
    for _idx, row in pd.concat([high_invalid, low_invalid, high_low_invalid]).head(10).iterrows():
...
def validate_data(symbol: str, timeframe: str, verbose: bool = False) -> dict:
    ...
    # Load data
    data_file = REPO_ROOT / "data" / "candles" / f"{symbol}_{timeframe}.parquet"
