...
1) **Risk för felaktig HTF-exit aktivering**: new vs legacy HTF exit-engine väljs av env/konfig-policy som kan ge olika beteende mellan runner och manuella körningar.
...
### Fynd A — HTF exit engine selection kan bli inkonsekvent mellan körmiljöer
**Fil:** `src/core/backtest/engine.py` (`_init_htf_exit_engine`)
...
**Risk**
- Manuell backtest som skickar `htf_exit_config={}` (tom dict) kan hamna i legacy trots att man tänkte "new".
- Runner som alltid injectar non-empty `htf_exit_config` kan få new engine utan att man explicit förstått det.

**Rekommenderad åtgärd**
- Kräv explicit opt-in via env eller en tydlig konfig-flagga (t.ex. `configs["htf_exit_config"]["enabled"]`).
- Logga alltid decision: env_flag, config_present, engine_selected.

**Severity:** Medium

---

### Fynd B — Config mutation per bar: kräver strikt isolering
**Fil:** `src/core/backtest/engine.py` (`run`)
...
### Fynd C — Precompute cache: nyckel bra, men edge cases kvar
**Fil:** `src/core/backtest/engine.py` (`_precompute_cache_key`, `_precompute_cache_key_material`)
...
### Fynd D — Error policy: “continue on error” i loop men raise efteråt
**Fil:** `src/core/backtest/engine.py` (`run`)
...
### Fynd E — HTF-context källa kan divergera (precomputed mapping vs meta)
**Fil:** `src/core/backtest/engine.py` (`_check_htf_exit_conditions`)
...
### Fynd F — Timestamp-typer: blandning av pandas.Timestamp och numpy-dtypes
**Fil:** `src/core/backtest/engine.py` (`run`)
