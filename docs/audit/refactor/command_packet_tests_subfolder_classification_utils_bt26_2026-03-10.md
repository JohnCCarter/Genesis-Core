...
- **Risk:** `LOW` — why: single-candidate test relocation av path-ren indikator-test utan aktiva docs-referenser.
- **Required Path:** `Full`
- **Objective:** Relocate `test_bollinger.py` to `tests/utils/` with no behavior change.
- **Candidate:** `tests/test_bollinger.py -> tests/utils/test_bollinger.py`
- **Base SHA:** `c70de683`
- **Category:** `tooling`
...
- **Candidate:** `test_bollinger.py`
- **Base SHA:** `c70de683`

### Scope

- **Scope IN:**
  - `tests/test_bollinger.py` (move source)
  - `tests/utils/test_bollinger.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (batch progress row)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt26_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_bollinger.py`
...
- `git rev-parse --short HEAD` -> `c70de683`

### Pre-coding verification evidence

- Candidate file inspected (`tests/test_bollinger.py`): inga `__file__`-referenser, ingen relativ fil-I/O eller cwd-beroende.
- `python -m pytest --collect-only -q tests/test_bollinger.py` -> `23` tests collected.
- `rg -n "tests/test_bollinger.py" docs --glob "!docs/archive/**" --glob "!docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt26_2026-03-10.md"` -> inga träffar (exit code `1`, expected for no matches).
- `tests/utils/test_bollinger.py` saknas före flytt (ingen target-kollision).

### Review + verification evidence

- Opus pre-code review: **APPROVED** (`Opus 4.6 Governance Reviewer`, BT26 pre-code review).
  - Scope/constraints/gates accepted utan extra remediation.
- Gates: **PASS**
  ...
  - `C:/Users/fa06662/Projects/Genesis-Core/.venv/Scripts/python.exe -m pytest -q tests/utils/test_bollinger.py` -> PASS (`23 passed`)
...
- Opus post-diff audit: **APPROVED** (`Opus 4.6 Governance Reviewer`, BT26 post-diff audit).
  - No mandatory remediation.
  - Cosmetic note addressed: READY_FOR_REVIEW set to complete.

### Implementation Report (BT26)

- **Scope summary (IN):**
  - Relokerade `tests/test_bollinger.py` -> `tests/utils/test_bollinger.py` (path-only; testkod oförändrad).
  - Uppdaterade planrad i `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` med BT26.
  - Dokumenterade kontrakt + gate evidence i denna packet-fil.
- **Scope summary (OUT):**
  - Ingen ändring i `src/**`, `mcp_server/**`, `scripts/**`, `config/**`, `.github/workflows/**`.
  - Ingen ytterligare testfil flyttad i BT26.
- **File-level changes:**
  - `tests/test_bollinger.py` -> deleted (move source)
  - `tests/utils/test_bollinger.py` -> added (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` -> BT26-rad tillagd
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt26_2026-03-10.md` -> packet + gate evidence
...
- **Artifacts:** BT26 packet + flyttdiff + planuppdatering + gate logs (terminal output)
