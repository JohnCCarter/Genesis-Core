diff --git a/docs/audit/STRATEGY_LOGIC_AUDIT.md b/docs/audit/STRATEGY_LOGIC_AUDIT.md
new file mode 100644
index 0000000..c5a7b4d
--- /dev/null
+++ b/docs/audit/STRATEGY_LOGIC_AUDIT.md
@@ -0,0 +1,155 @@
...
- +---
- +## Detaljfynd
- +### Fynd A — Kritisk: `float(None)` kan fortfarande krascha (probas & thresholds) +**Filer:**
  +- `src/core/strategy/decision.py`
  +- `src/core/strategy/confidence.py`
- +**Observation**
...
- +**Severity:** High
- +---
- +### Fynd B — Konfig- och typkontrakt är implicit: champion+override merge kan föra in "råa" värden +**Fil:** `src/core/strategy/evaluate.py`
- +**Observation**
...
- +---
- +### Fynd C — Monolitisk `decide()` gör logiken svårtestad och riskerar regressions +**Fil:** `src/core/strategy/decision.py`
- +**Observation**
...
- +**Severity:** Medium
- +---
- +### Fynd D — Override-state och debug kan växa: risk för prestanda/IO +**Fil:** `src/core/strategy/decision.py`
- +**Observation**
...
- +**Severity:** Low–Medium
- +---
- +### Fynd E — Dubbel regimlogik (fast-path vs unified) kan ge inkonsekvenser +**Fil:** `src/core/strategy/evaluate.py`
- +**Observation**
  +- Regim beräknas antingen via precomputed EMA50 fast-path eller `detect_regime_unified()`.
  +- Fast-path och `detect_regime_unified()` använder samma ±2%-trösklar för EMA-avvikelse; kvarvarande risk ligger främst i index/datakälla (`_global_index` + fullserie/precomputed vs lokalt fönster).
- +**Risk**
  +- Samma marknadsdata kan få olika regime om indexering/datakälla inte är alignad mellan fast-path och unified-läget → påverkar kalibrering och gating.
- +**Rekommenderad åtgärd**
  +- Verifiera paritet med riktat test som jämför fast-path och unified för samma tidssteg/index, och dokumentera eventuella avvikelser som index-/datakällespecifika.
- +**Severity:** Medium
- +---
