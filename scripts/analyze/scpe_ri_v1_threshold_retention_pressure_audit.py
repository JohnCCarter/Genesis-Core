#!/usr/bin/env python3
"""Observational threshold-retention pressure audit for frozen RI replay evidence.

This script is RESEARCH-only and observational-only. It profiles the raw defensive
candidates that are retained in continuation via `confidence_below_threshold` using a
research-side analytical mirror of the existing replay behavior. It does not modify
canonical replay logic or propose a new threshold rule.
"""
...
AUDIT_VERSION = "scpe-ri-v1-threshold-retention-pressure-audit-2026-04-20"
...
OUTPUT_PATH = (
    ...
    / "results"
    / "evaluation"
    / "scpe_ri_v1_threshold_retention_pressure_audit_2026-04-20.json"
)
INPUT_PATHS = {
...
def main() -> int:
    ...
    payload = {
        ...
        "mode": "RESEARCH",
        "status": "threshold-retention-pressure-audit-generated",
        ...
        "source_hashes": {
            **{_relative_path(path): meta["sha256"] for path, meta in []},
            **{_relative_path(path): _sha256_file(path) for path in [Path(__file__).resolve()]},
        },
        ...
        "threshold_examples_sorted": _sort_examples(threshold_retained_rows)[:5],
        "comparison_deltas": {
            "threshold_vs_raw_defensive": _delta_summary(
                comparator_summaries["threshold_retained"],
                comparator_summaries["raw_defensive_candidates"],
            ),
            "threshold_vs_min_dwell": _delta_summary(
                comparator_summaries["threshold_retained"],
                comparator_summaries["min_dwell_retained"],
            ),
            "threshold_vs_selected_defensive": _delta_summary(
                comparator_summaries["threshold_retained"],
                comparator_summaries["selected_defensive"],
            ),
        },
        "descriptive_guardrails": {
            "report_only_interpretation_labels": ["near-miss", "materially weaker", "mixed"],
            "script_emits_metrics_only": True,
            "no_threshold_rule_proposal": True,
            "no_replay_quality_reinterpretation": True,
        },
        "findings": [
            "Threshold-retained rows are emitted as a descriptive comparator bucket only; the script does not classify them as near-miss or materially weaker.",
    ...
    if replay_root_diff:
        raise RuntimeError(
            "Baseline replay root changed during threshold retention pressure audit: "
            + json.dumps(replay_root_diff, ensure_ascii=False, indent=2)
        )

    print(f"[OK] Wrote threshold retention pressure artifact: {OUTPUT_PATH}")
    print(f"[OK] Threshold-retained rows: {len(threshold_retained_rows)}")
    print(f"[OK] Selected-defensive comparator rows: {len(selected_defensive_rows)}")
    return 0
