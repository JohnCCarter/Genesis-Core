# Feature Attribution v1 — execution slice 1 packet

Date: 2026-03-31
Branch: `feature/ri-role-map-implementation-2026-03-24`
Status: `historical Feature Attribution v1 execution slice 1 packet / retained for provenance / not current branch guidance`
...
- **Required Path:** `Full`
- **Objective:** Execute the first governed Feature Attribution v1 identification slice for `Minimum-edge gate seam` only and classify it descriptively as `additive`, `neutral`, `adverse`, `inconclusive`, or `invalid` against the locked baseline.
- **Candidate:** `Minimum-edge gate seam`
- **Base SHA:** `68537da2`

### Scope

- **Scope IN:** this execution packet; one explicit single-row execution slice for `Minimum-edge gate seam` only; locked baseline provenance from `results/research/fib_baseline_backtest_v1/summary.md` and `results/research/fib_baseline_backtest_v1/metrics.json`; existing internal execution path only (`src/core/pipeline.py`, `src/core/backtest/engine.py`, `src/core/strategy/evaluate.py`, `src/core/strategy/decision.py`) with no code changes; one human-readable report under `results/research/feature_attribution_v1/reports/`; one machine-readable manifest under `results/research/feature_attribution_v1/manifests/`.
- **Scope OUT:** no source-code changes; no tests added/edited; no new CLI surface; no new env-var surface; no runner expansion; no execution of any row other than `Minimum-edge gate seam`; no cluster execution; no multi-row ranking; no promotion/cutover semantics; no edits under `config/strategy/champions/**`; no fib reopening; no artifacts outside the explicitly named report/manifest outputs.
- **Expected changed files:** `docs/decisions/feature_attribution/v1/feature_attribution_v1_exec_slice1_min_edge_identification_packet_2026-03-31.md`, `results/research/feature_attribution_v1/reports/fa_v1_min_edge_20260331_01.md`, `results/research/feature_attribution_v1/manifests/fa_v1_min_edge_20260331_01.json`
- **Max files touched:** `3`
...
| Field                   | Value                                                     |
| ----------------------- | --------------------------------------------------------- |
| `run_id`                | `fa_v1_min_edge_20260331_01`                              |
...
- Required SPEC anchors:
  ...
  - `.github/skills/feature_parity_check.json` — conditional parity anchor; must be invoked and then recorded as not runtime-authoritative for this slice because the selected row does not touch feature-computation surfaces
...
- `policy["feature_attribution"]["selected_row_label"] = "Minimum-edge gate seam"`
...
- `python -m pytest -q tests/utils/test_decision_edge.py tests/utils/test_decision.py`
...
- manual effective-config check that the only intentional candidate diff is neutralization of `thresholds.min_edge`
...
- any gate failure
- any evidence that the effective-config diff escapes `thresholds.min_edge`
- any attempt to rank multiple rows or turn the descriptive label into a direct action

### Output required

- one executed single-row candidate-vs-baseline comparison for `Minimum-edge gate seam`
- one descriptive classification using the locked Phase 5 vocabulary
- one report at `results/research/feature_attribution_v1/reports/fa_v1_min_edge_20260331_01.md`
- one manifest at `results/research/feature_attribution_v1/manifests/fa_v1_min_edge_20260331_01.json`
...
- `candidate_effective_min_edge`
- `baseline_effective_min_edge`
...
This packet authorizes only the first identification slice.
