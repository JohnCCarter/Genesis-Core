...
def test_feature_ic(
    ...
    # Q5-Q1 spread
    if len(quintile_returns) >= 5 and not np.isnan(quintile_returns[4]) and not np.isnan(quintile_returns[0]):
...
def main():
    ...
    try:
        ...
        print(f"[REGIME] {args.regime} samples: {regime_mask.sum():,}/{len(regime_mask):,} ({regime_mask.mean():.1%})")
        ...
        for rank, (feat_name, stats) in enumerate(sorted_results, 1):
            ...
            print(f"{rank:<5} | {feat_name:<30} | {ic_str:<8} {ic_rating:<12} | {spread_str:<8} | {sig_str}")
        ...
        for rank, (feat_name, stats) in enumerate(sorted_results[:10], 1):
            ...
            print(f"   Spread:   {stats['q5_q1_spread']:+.3%}" if stats['q5_q1_spread'] else "   Spread:   N/A")
