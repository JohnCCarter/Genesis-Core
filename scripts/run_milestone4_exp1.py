#!/usr/bin/env python3
"""
Milestone 4 - Experiment 4.1: EVGate Quality Tuning

Single config change: EVGate min_ev 0.0 -> 0.05 (careful threshold)
...
def hqt_audit_with_comparison(results: dict, baseline_results: dict = None):
    ...
    print(f"\n{'='*90}")
    print("HQT AUDIT - Milestone 4 Experiment 4.1")
    print(f"{'='*90}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Milestone 4 Experiment 4.1")
    parser.add_argument(
        "--config",
        default="config/strategy/composable/phase2/v6a_evgate_exp1.yaml",
        help="Path to config",
    )
    parser.add_argument(
        "--baseline",
