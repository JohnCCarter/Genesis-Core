...
> 2026-05-18 note (`feature/risk-hardening-wave2`): This audit remains a historical finding report from 2026-02-21.
>
> - Current repo behavior still matches the narrow live-update boundary enforced by `ConfigAuthority.propose_update()`.
> - That current executable boundary is also covered by `tests/governance/test_config_ssot.py`, `tests/integration/test_config_endpoints.py`, and `tests/integration/test_config_api_e2e.py`.
> - Current governed reading on the repo surface is **B1**: keep the stricter live whitelist and document it clearly.
> - **B2/B3** remain separate behavior-change candidates; this historical audit note does not approve whitelist expansion.
...
2. **Runtime update-whitelist** i `ConfigAuthority.propose_update()` är strikt och utesluter fält som runtime-schemat faktiskt stödjer (t.ex. `exit`, `features`) → kan uppfattas som fel/felkonfigurerat API om avsikten är att dessa ska kunna ändras live; på dagens repo-yta är detta dock en dokumenterad live-write-asymmetri, inte implicit bevis för att dessa fält redan ska vara live-updatable.
