...
def analyze_current_models():
    ...
    for model_file in models_dir.glob("*.json"):
        ...
        try:
            ...
            problems.append({"file": model_file.name, "issue": f"JSON parse error: {e}"})
            ...
            if isinstance(value, dict) and "champion" in value:
                ...
                if not os.path.exists(champion_path):
                    registry_issues.append(
                        {"key": key, "issue": f"Champion model saknas: {champion_path}"}
                    )
                elif "tBTCUSD.json" in champion_path:
                    registry_issues.append(
                        {"key": key, "issue": f"Pekar på gammal model: {champion_path}"}
                    )
    ...
    for issue in registry_issues:
        if "Pekar på gammal model" in issue["issue"]:
            update_plan["registry_updates"].append(
                {"key": issue["key"], "action": "Uppdatera till ny model"}
            )
    ...
    for problem in problems:
        if problem.get("timeframe") == "4h":
            update_plan["timeframe_fixes"].append(
                {"file": problem["file"], "timeframe": "4h", "action": "Byt till 3h eller 6h"}
            )
