...
2. if the user wants one more hardening step on the findings bank, the cheapest next move is a tiny tooling follow-up that deterministically rebuilds or validates `artifacts/research_ledger/indexes/findings_index.json` from the committed findings-bank sources; do not widen `artifacts/` further and do not touch CI unless a clean-checkout gap is actually reproduced first
3. if the user wants another RI-router slice, keep it docs-only and re-anchor it to fresh active-carrier evidence plus the annual evidence notes before proposing any new candidate family or runtime packet; the cheapest RI follow-up after the same-window note is a phase-ordering pass on these same fixed `2020` and `2018` windows, not widening to more years and not any new runtime tuning
