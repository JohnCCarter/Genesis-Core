diff --git a/docs/audit/CONFIG_GOVERNANCE_AUDIT.md b/docs/audit/CONFIG_GOVERNANCE_AUDIT.md
new file mode 100644
index 0000000..c7f1c2a
--- /dev/null
+++ b/docs/audit/CONFIG_GOVERNANCE_AUDIT.md
@@ -0,0 +1,134 @@
...
- +1) **Två parallella konfig-system** (legacy JSON-schema vs runtime Pydantic) utan tydlig separation → hög risk för felvalidering och felaktiga antaganden.
  +2) **Runtime update-whitelist** i `ConfigAuthority.propose_update()` är strikt och utgör en avsiktlig policygräns för live-uppdateringar; den skiljer sig från runtime-schemats fulla giltighetsyta (t.ex. `exit`, `features`) och kan skapa förväntansglapp om policyn inte dokumenteras tydligt.
...
- +## Detaljfynd
- +### Fynd A — Parallella config-system utan tydlig kontraktgräns +**Filer:**
...
- +---
- +### Fynd B — `ConfigAuthority` whitelist blockerar runtime-fält som schemat stödjer +**Fil:** `src/core/config/authority.py`
- +**Observation**
  +- Whitelisten i `ConfigAuthority.propose_update()` är en avsiktlig policygräns för live-uppdateringar.
...
- +**Risk**
  +- Om policygränsen inte är tydligt dokumenterad kan API/ops tolka detta som inkonsistent beteende.
  +- Förväntansglapp: team kan göra ändringar som valideras via Pydantic men stoppas i propose-vägen.
...
- +---
- +### Fynd C — Governance registry: risk för falska konflikter p.g.a. dubbletter i `active_compacts` +**Fil:** `src/core/governance/registry.py`
- +**Observation**
...
- +---
- +### Fynd D — `validator.diff_config()` är toppnivå-only och kan missleda +**Fil:** `src/core/config/validator.py`
- +**Observation**
...
- +**Severity:** Low–Medium
- +---
- +### Fynd E — Små validerings-edge cases i runtime-schema +**Fil:** `src/core/config/schema.py`
- +**Observation / Risk**
  +- `Risk.risk_map`: validerar struktur och float-coercion men inte monotonicitet/bounds (t.ex. storlek > 1 kan smyga in).
  +- `LTFOverrideAdaptiveConfig.regime_multipliers` är typat som `dict[str, float]` och coerces i `RuntimeConfig`-valideringen; praktisk risk uppstår främst i lagergränsen mellan whitelistens strukturkontroll och senare schemavalidering.
- +**Rekommenderad åtgärd**
  +- Lägg till validering:
- - risk_map: enforce `0<=thr<=1` och `0<=size<=1`, samt ev sortering/monotonicitet.
- - regime_multipliers: lägg riktad test för icke-numerisk input via propose-vägen och säkerställ tydligt felmeddelande.
- +**Severity:** Low
