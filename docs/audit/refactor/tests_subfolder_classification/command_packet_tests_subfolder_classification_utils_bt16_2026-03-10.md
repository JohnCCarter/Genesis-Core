...
- **Required Path:** `Full`
- **Objective:** Relocate `test_read_helpers.py` to `tests/utils/` with no behavior change.
- **Candidate:** `tests/test_read_helpers.py -> tests/utils/test_read_helpers.py`
- **Base SHA:** `ad7297e0`
- **Category:** `tooling`
...
- **Candidate:** `test_read_helpers.py`
- **Base SHA:** `ad7297e0`

### Scope

- **Scope IN:**
  - `tests/test_read_helpers.py` (move source)
  - `tests/utils/test_read_helpers.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (batch progress row)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt16_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_read_helpers.py`
...
- Path-reference check i `docs/**` gav inga träffar utanför denna packet-fil; inga ytterligare icke-arkiv-referenser kräver uppdatering i BT16.
- `rg -n "tests/test_read_helpers.py" docs --glob "!docs/archive/**" --glob "!docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt16_2026-03-10.md"` -> inga träffar (exit code `1`, expected for no matches).
...
- `python -m pytest -q tests/utils/test_read_helpers.py`: **PASS** (`3 passed`)
...
- Note: initial run surfaced `ruff` W292 on moved target; remediated via controlled file rewrite with terminal newline and full gate stack rerun to PASS.

### Implementation Report (BT16)

- **Scope summary (IN):** Flytt av `tests/test_read_helpers.py` till `tests/utils/`, planuppdatering och BT16-packet.
...
- **Artifacts:** BT16 packet + flyttdiff + planuppdatering
