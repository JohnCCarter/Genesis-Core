            ...
            if os.path.exists(champion_path):
                try:
                    ...
                    if "schema" in model_data and len(model_data["schema"]) >= 5:
                        ...
                        print(
                            f"ERROR: {key} - Fel schema: {len(model_data.get('schema', []))} features"
                        )
                        error_count += 1
...
def test_model_registry_lookup():
    ...
    try:
        ...
        # Testa några lookups
        test_cases = [("tBTCUSD", "1h"), ("tETHUSD", "6h"), ("tADAUSD", "1D"), ("tSOLUSD", "1W")]
...
def test_probability_prediction():
    ...
    try:
        ...
        if "buy" in probas and "sell" in probas and "hold" in probas:
            print(
                f"OK: Prediction fungerar - Buy: {probas['buy']:.3f}, Sell: {probas['sell']:.3f}, Hold: {probas['hold']:.3f}"
            )
            return True
        else:
