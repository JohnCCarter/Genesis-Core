...
- **Risk:** `LOW` — why: single-candidate test relocation av path-ren env-flag-test utan aktiva docs-referenser.
- **Required Path:** `Full`
- **Objective:** Relocate `test_env_flags.py` to `tests/utils/` with no behavior change.
- **Candidate:** `tests/test_env_flags.py -> tests/utils/test_env_flags.py`
- **Base SHA:** `9fde9198`
- **Category:** `tooling`
...
- **Candidate:** `test_env_flags.py`
- **Base SHA:** `9fde9198`

### Scope

- **Scope IN:**
  - `tests/test_env_flags.py` (move source)
  - `tests/utils/test_env_flags.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (batch progress row)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt24_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_env_flags.py`
...
- Attempt with repo-local venv `C:/Users/fa06662/Projects/Genesis-Core-refactor-b/.venv/Scripts/python.exe ...` -> `ModuleNotFoundError: jsonschema` (non-zero exit).
...
- `git rev-parse --short HEAD` -> `9fde9198`

### Pre-coding verification evidence

- Candidate file inspected (`tests/test_env_flags.py`): inga `__file__`-referenser, ingen relativ fil-I/O eller cwd-beroende.
- `python -m pytest --collect-only -q tests/test_env_flags.py` -> `13` tests collected.
- `rg -n "tests/test_env_flags.py" docs --glob "!docs/archive/**" --glob "!docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt24_2026-03-10.md"` -> inga träffar (exit code `1`, expected for no matches).
- `tests/utils/test_env_flags.py` saknas före flytt (ingen target-kollision).

### Review + verification evidence

- Opus pre-code review: **APPROVED_WITH_NOTES** (`Opus 4.6 Governance Reviewer`, BT24 pre-code review).
  - Scope/constraints/gates accepted som governance-kompatibla.
  - Note addressed: skill-evidence provenance clarified till aktuell workspace-cwd.
- Gates: **PASS**
  ...
  - `C:/Users/fa06662/Projects/Genesis-Core/.venv/Scripts/python.exe -m pytest -q tests/utils/test_env_flags.py` -> PASS (`13 passed`)
...
- Opus post-diff audit: **APPROVED** (`Opus 4.6 Governance Reviewer`, BT24 post-diff audit).
  - No mandatory remediation.
  - Note observed: rerun gate-set only if new diff appears before push.

### Implementation Report (BT24)

- **Scope summary (IN):**
  - Relokerade `tests/test_env_flags.py` -> `tests/utils/test_env_flags.py` (path-only; testkod oförändrad).
  - Uppdaterade planrad i `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` med BT24.
  - Dokumenterade kontrakt + gate evidence i denna packet-fil.
- **Scope summary (OUT):**
  - Ingen ändring i `src/**`, `mcp_server/**`, `scripts/**`, `config/**`, `.github/workflows/**`.
  - Ingen ytterligare testfil flyttad i BT24.
- **File-level changes:**
  - `tests/test_env_flags.py` -> deleted (move source)
  - `tests/utils/test_env_flags.py` -> added (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` -> BT24-rad tillagd
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt24_2026-03-10.md` -> packet + gate evidence
...
- **Artifacts:** BT24 packet + flyttdiff + planuppdatering + gate logs (terminal output)
