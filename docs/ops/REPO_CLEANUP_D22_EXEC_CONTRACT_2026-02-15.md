# Repo Cleanup D22 Minimal Delete Execution Contract (2026-02-15)
...
- Delete-only execution av exakt tre föråldrade backtestfiler (äldsta 1h-filerna):
...
- `.gitignore`
- `tmp/**`
- `results/**` (förutom exakt de tre scopeade filerna)
...
- Scopeade filer är de äldsta 1h-backtestfilerna i `results/backtests` (2025-10-22).
- Basename-referenser finns i legacy run-artefakter (`results/hparam_search/phase7b_optuna/**`) och noteras som residual risk.
...
1. Exakt tre scopeade backtestfiler raderade.
2. D22 kontrakt + rapport skapade.
3. Backlog + `AGENTS.md` uppdaterade med korrekt statusdisciplin.
4. Diffen är strikt scopead till 3 raderingar + docs i Scope IN.
5. Required gates passerar efter ändring.
6. Opus post-code diff-audit är `APPROVED`.

## Status

- D22 i denna tranche är en minimal delete-only batch.
