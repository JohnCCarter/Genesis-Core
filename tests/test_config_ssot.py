...
from core.config.authority import ConfigAuthority, RUNTIME_PATH
...
def test_config_roundtrip_and_hash(tmp_path: Path) -> None:
    ...
    assert ver1 == 0
    snap = auth.propose_update({"thresholds": {"entry_conf_overall": 0.5}}, actor="t", expected_version=0)
...
def test_only_whitelisted_keys_change(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError):
        auth.propose_update({"features": {"percentiles": {"rsi": [-1, 1]}}}, actor="t", expected_version=0)
