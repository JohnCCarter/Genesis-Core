...
- mode #1 (evidence-to-authority drift) är öppet erkänd som branchens rank-1 risk i 2026-05-15-djup-premortem. Den är **inte mitigerad** av denna doc; den är bara dokumenterad oftare. Mitigationen sker per-citation, inte per-doc.
  ...
  Later-branch residual-scope note (2026-05-19, `feature/risk-hardening-wave2`): the `#1` sentence above remains directionally open, but its honest current-branch shape should be read more narrowly than `one already-identified small seam still waiting to be patched`. Current branch-visible packets already consumed the clearest adjacent tracked docs seams around citation framing, stale active-lane pointers, paused editor-worker references, premortem closeout scope, and premortem anchor-role competition. What remains for `#1` is therefore the broader family-level retelling/adoption risk itself: bounded observational notes can still be over-read later even after those smaller seams were corrected. This does **not** close `#1`; it clarifies that the next honest bounded `#1` slice would first require fresh selection of a specific still-misleading, frequently reused claim-bearing surface rather than reusing already-corrected seams. See `docs/decisions/governance/evidence_to_authority_drift_residual_scope_reanchor_packet_2026-05-19.md`.
...
- mode #8 + #9 (HTF-relaterade) handlar båda om att samma logiska intent (kör HTF-exits) realiseras via olika kod-paths beroende på empty-dict vs explicit-enabled vs precomputed-mapping. Audit-rekommendationen är `htf_exit_config["enabled"]` opt-in.
  ...
  Later-branch partial reclassification note (2026-05-19, `feature/risk-hardening-wave2`): the exact `#9` carry-forward reading above should also be narrowed for the tracked `BacktestEngine` HTF consumer path. Current `src/core/backtest/engine.py::_check_htf_exit_conditions()` now behaves as `precomputed-or-unavailable` when HTF mapping is present: invalid `0.0/NaN` or non-positive mapped values are sanitized to unavailable, invalid swing bounds force unavailable, and out-of-range precomputed access is caught and kept unavailable rather than silently adopting `meta["features"]["htf_fibonacci"]`. Focused selectors in `tests/backtest/test_backtest_engine.py` now pass for invalid precomputed context, valid precomputed context, and out-of-range precomputed access with valid meta fallback present. This narrows the current residual to broader cross-path equivalence and future producer/consumer drift, not an unchanged live-branch reading of silent fake-HTF or silent-meta-fallback behavior for the tracked consumer path. See `docs/decisions/governance/backtest_htf_context_divergence_partial_reclassification_packet_2026-05-19.md`.
...
- mode #13 (lookahead-leak under `GENESIS_FAST_WINDOW=1`) är **strukturell hypotes** (C). `GENESIS_FAST_WINDOW=1` är canonical default per CLAUDE.md, och precompute-features är också canonical default (`GENESIS_PRECOMPUTE_FEATURES=1`). Båda har potentiella ordning-känsligheter. Ingen tracked test sett som verifierar slow-path vs fast-path identitet per bar.
  ...
  Later-branch partial reclassification note (2026-05-19, `feature/risk-hardening-wave2`): the exact `#13` blanket reading above is now too strong for this checkout. Current branch-visible evidence already includes `tests/integration/test_precompute_vs_runtime.py::test_precompute_features_match_runtime`, `tests/utils/test_features_asof_fast_window_parity.py::test_extract_features_backtest_is_prefix_invariant_to_future_bar_mutation`, `tests/utils/test_features_asof_fast_window_parity.py::test_extract_features_backtest_remapped_precompute_is_prefix_invariant_on_fast_window`, and `tests/backtest/test_backtest_engine.py::test_build_candles_window_fast_window_matches_numpy_path`; this slice adds a direct same-global-bar parity regression comparing full runtime extraction against fast-window remapped precompute extraction on the same bar within bounded-window recursive-indicator tolerance rather than claiming exact full-history identity from a truncated 200-bar window. That does not close the broader family, but it narrows the current residual to lack of exhaustive parity coverage and lack of exact full-history identity guarantees across more bars/timeframes/path permutations, not a total absence of tracked parity proof. See `docs/decisions/governance/fast_window_same_bar_parity_partial_reclassification_packet_2026-05-19.md`.
...
- mode #14 (decision-coercion crash) är också C — ingen fuzz/property-test sett för `decide()`-input. Pydantic v2 ger viss skydd, men `decide()` tar dict-baserade inputs i delar av call-graph.
  ...
  Later-branch partial reclassification note (2026-05-19, `feature/risk-hardening-wave2`): the exact `#14` crash reading above should now be narrowed on current branch-visible evidence. Current `src/core/strategy/decision_gates.py` already fail-safes missing or non-dict `probas` in `select_candidate(...)`, fail-safes missing or non-dict `confidence` in `apply_post_fib_gates(...)`, and uses `safe_float(...)` for non-numeric scalar coercion rather than raising. Existing tests in `tests/utils/test_decision.py` and `tests/utils/test_decision_gates_contract.py`, plus the additional non-dict input-shape coverage added in this slice, now prove that several malformed `probas` / `confidence` shapes return fail-safe `NONE` instead of crashing. This narrows the current residual to broader property-based or more exhaustive shape coverage, not an unchanged branch-visible `None` / string input crash seam. See `docs/decisions/governance/decision_input_fail_safe_partial_reclassification_packet_2026-05-19.md`.
