...
- **Required Path:** `Full`
- **Objective:** Relocate `test_mcp_blocked_patterns_windows_path.py` to `tests/utils/` with no behavior change.
- **Candidate:** `tests/test_mcp_blocked_patterns_windows_path.py -> tests/utils/test_mcp_blocked_patterns_windows_path.py`
- **Base SHA:** `c6f59be4`
- **Category:** `tooling`
...
- **Candidate:** `test_mcp_blocked_patterns_windows_path.py`
- **Base SHA:** `c6f59be4`

### Scope

- **Scope IN:**
  - `tests/test_mcp_blocked_patterns_windows_path.py` (move source)
  - `tests/utils/test_mcp_blocked_patterns_windows_path.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (batch progress row)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt20_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_mcp_blocked_patterns_windows_path.py`
...
- Path-reference check i `docs/**` gav inga träffar utanför denna packet-fil; inga ytterligare icke-arkiv-referenser kräver uppdatering i BT20.
- `rg -n "tests/test_mcp_blocked_patterns_windows_path.py" docs --glob "!docs/archive/**" --glob "!docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt20_2026-03-10.md"` -> inga träffar (exit code `1`, expected for no matches).
...
- `python -m pytest -q tests/utils/test_mcp_blocked_patterns_windows_path.py`: **PASS** (`1 passed`)
...
- Note: first run surfaced `ruff` W292 in moved target; remediated with trailing newline and full gate stack rerun to PASS.

### Implementation Report (BT20)

- **Scope summary (IN):** Flytt av `tests/test_mcp_blocked_patterns_windows_path.py` till `tests/utils/`, planuppdatering och BT20-packet.
...
- **Artifacts:** BT20 packet + flyttdiff + planuppdatering
