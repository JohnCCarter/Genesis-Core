...
- **Mode:** `RESEARCH` — source: `docs/governance_mode.md` (`branch: feature/tests-subfolder-classification-bt35`)
- **Risk:** `LOW` — why: single-candidate utility-test relocation utan runtime- eller configpåverkan.
- **Required Path:** `Full`
- **Objective:** Relocate `test_exchange_client.py` till `tests/utils/` med no-behavior-change.
- **Candidate:** `tests/test_exchange_client.py -> tests/utils/test_exchange_client.py`
- **Base SHA:** `7877ce46`
- **Category:** `tooling`
...
- **Risk:** `LOW` — why: isolated exchange-client utility-test utan path/cwd/IO-känslighet
- **Required Path:** `Full`
- **Objective:** Continue utils-category migration med låg-risk-kandidat.
- **Candidate:** `test_exchange_client.py`
- **Base SHA:** `7877ce46`

### Scope

- **Scope IN:**
  - `tests/test_exchange_client.py` (move source)
  - `tests/utils/test_exchange_client.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (BT35-rad)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt35_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_exchange_client.py`
...
- `repo_clean_refactor` (`föreslagen` skill-tillämpning för scope/no-behavior-change-guardrails)
...
- Candidate file inspected (`tests/test_exchange_client.py`): inga `__file__`-referenser, ingen relativ fil-I/O eller cwd-beroende.
- `python -m pytest --collect-only -q tests/test_exchange_client.py` -> `2` tests collected.
- Repo-wide path-reference check: inga externa träffar utanför arkiverade docs för `tests/test_exchange_client.py`.
- `tests/utils/test_exchange_client.py` saknas före flytt (ingen target-kollision).
...
- Gates: **PASS**
  ...
  - `python -m pytest -q tests/utils/test_exchange_client.py` -> PASS (`2 passed`)
...
### Implementation Report (BT35)

- **Scope summary (IN):**
  - Relokerade `tests/test_exchange_client.py` -> `tests/utils/test_exchange_client.py` (path-only, oförändrat testinnehåll).
  - Uppdaterade `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` med BT35-rad.
...
- **Scope summary (OUT):**
  ...
  - Ingen ytterligare testflytt utöver BT35-kandidaten.
- **File-level changes:**
  - `tests/test_exchange_client.py` -> deleted (move source)
  - `tests/utils/test_exchange_client.py` -> added (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` -> BT35-rad tillagd
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt35_2026-03-10.md` -> packet + evidence uppdaterad
...
- **Artifacts:** BT35 packet + flyttdiff + planuppdatering + gate logs
