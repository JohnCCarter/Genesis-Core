...
- **Required Path:** `Full`
- **Objective:** Relocate `test_gate_dominance_utils.py` to `tests/utils/` with no behavior change.
- **Candidate:** `tests/test_gate_dominance_utils.py -> tests/utils/test_gate_dominance_utils.py`
- **Base SHA:** `424175fa`
- **Category:** `tooling`
...
- **Candidate:** `test_gate_dominance_utils.py`
- **Base SHA:** `424175fa`

### Scope

- **Scope IN:**
  - `tests/test_gate_dominance_utils.py` (move source)
  - `tests/utils/test_gate_dominance_utils.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (batch progress row)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt14_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_gate_dominance_utils.py`
...
- Path-reference check i `docs/**` gav inga träffar utanför denna packet-fil; inga ytterligare icke-arkiv-referenser kräver uppdatering i BT14.
- `rg -n "tests/test_gate_dominance_utils.py" docs --glob "!docs/archive/**" --glob "!docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt14_2026-03-10.md"` -> inga träffar (exit code `1`, expected for no matches).

### Review + verification evidence

- Opus pre-code review: **APPROVED_WITH_NOTES** -> notes remediated (wording + explicit docs-grep evidence) -> **GO**.
- `python -m pre_commit run --all-files`: **PASS**
- `python -m ruff check .`: **PASS**
- `python -m pytest -q tests/utils/test_gate_dominance_utils.py`: **PASS** (`4 passed`)
...
### Implementation Report (BT14)

- **Scope summary (IN):** Flytt av `tests/test_gate_dominance_utils.py` till `tests/utils/`, planuppdatering och BT14-packet.
...
- **Artifacts:** BT14 packet + flyttdiff + planuppdatering
