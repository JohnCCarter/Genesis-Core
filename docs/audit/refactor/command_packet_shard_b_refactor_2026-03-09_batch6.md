...
- **Objective:** Continue Shard B maintainability refactor by deduplicating repeated patch setup scaffolding in optimizer runner tests while preserving assertion parity.
- **Candidate:**
  - Extract helper for repeated `patch.dict(os.environ, {"GENESIS_MAX_CONCURRENT": "1"})`.
  - Extract helper for repeated `patch("core.optimizer.runner.RESULTS_DIR", ...)`.
  - Extract helper for repeated `patch("core.strategy.champion_loader.CHAMPIONS_DIR", _champions_dir(tmp_path))`.
- **Base SHA:** `3a7a63c6`
...
- **Scope IN:**
  ...
  - `docs/audit/refactor/command_packet_shard_b_refactor_2026-03-09_batch6.md`
- **Scope OUT:**
...
Current changed files are constrained to batch6 scope (`tests/test_optimizer_runner.py` + this packet).
