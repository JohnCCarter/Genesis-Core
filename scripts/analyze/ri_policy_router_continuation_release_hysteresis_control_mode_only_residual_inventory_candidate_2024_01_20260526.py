...
from scripts.analyze import (
    ri_policy_router_continuation_release_hysteresis_local_action_position_equivalence_candidate_2024_01_20260526 as action_2024_01,
)
from scripts.analyze import (
...
SUBJECT_IDS = (
    action_2024_01.NEGATIVE_CANDIDATE.subject_id,
    action_2024_01.POSITIVE_CONTROL.subject_id,
)
ACTION_POSITION_EQUIVALENCE_RELATIVE = Path(
    "results/evaluation/ri_policy_router_continuation_release_hysteresis_local_action_position_equivalence_candidate_2024_01_2026-05-26.json"
)
OUTPUT_ROOT_RELATIVE = Path("results/evaluation")
OUTPUT_FILENAME = "ri_policy_router_continuation_release_hysteresis_control_mode_only_residual_inventory_candidate_2024_01_2026-05-26.json"
STATUS_OK = "continuation_release_hysteresis_control_mode_only_residual_inventory_candidate_2024_01_generated"
STATUS_FAIL_CLOSED = "continuation_release_hysteresis_control_mode_only_residual_inventory_candidate_2024_01_fail_closed"
...
class Candidate202401ControlModeResidualInventoryError(RuntimeError):
    pass
...
def _load_action_position_subjects() -> dict[str, dict[str, Any]]:
    payload = local_packet._coerce_dict(
        ...
        field_name="candidate_2024_01_control_mode_only_inventory.action_position.payload",
    )
    subject_payloads = local_packet._coerce_dict(
        payload.get("subject_payloads"),
        field_name="candidate_2024_01_control_mode_only_inventory.action_position.subject_payloads",
    )
    ...
    for subject_id in SUBJECT_IDS:
        subject_payload = local_packet._coerce_dict(
            ...
            field_name=(
                f"candidate_2024_01_control_mode_only_inventory.action_position.subject_payloads.{subject_id}"
            ),
        )
        row_analyses = [
            candidate_replay._coerce_episode_row(
                raw_row,
                field_name=(
                    f"candidate_2024_01_control_mode_only_inventory.action_position.subject_payloads.{subject_id}.row[{index}]"
                ),
            )
            for index, raw_row in enumerate(
                local_packet._coerce_list(
                    ...
                    field_name=(
                        f"candidate_2024_01_control_mode_only_inventory.action_position.subject_payloads.{subject_id}.envelope_row_analysis"
                    ),
                )
            )
        ]
...
def _load_candidate_last_locked_size_timestamp(subject_payload: dict[str, Any]) -> str:
    ...
    if not locked_rows:
        raise Candidate202401ControlModeResidualInventoryError(
            "Candidate 2024-01 action-position artifact no longer contains locked-size rows"
        )
    ...
    *,
    ...
    serialized_rows = [
        _serialize_row(
            ...
            candidate_last_locked_size_timestamp=(
                candidate_last_locked_size_timestamp
                if subject_id == action_2024_01.NEGATIVE_CANDIDATE.subject_id
                else None
            ),
        )
    ...
    return {
        ...
        "all_rows_after_candidate_last_locked_size_row": (
            ...
            if subject_id == action_2024_01.NEGATIVE_CANDIDATE.subject_id and serialized_rows
            else None
        ),
...
def _build_packet_summary(subject_payloads: dict[str, dict[str, Any]]) -> dict[str, Any]:
    candidate_payload = subject_payloads[action_2024_01.NEGATIVE_CANDIDATE.subject_id]
    control_payload = subject_payloads[action_2024_01.POSITIVE_CONTROL.subject_id]
    ...
    if candidate_count == 0 and control_count == 0:
        ...
        inference = (
            "Neither the 2024-01 candidate nor the 2023-05 control retains any rows where `switch_control_mode` is "
        ...
        inference = (
            "Only the `2024-01` candidate retains rows where `switch_control_mode` is the sole remaining diff field. "
...
def _build_fail_closed_result(reason: str) -> dict[str, Any]:
    return {
        "audit_version": (
            "ri-policy-router-continuation-release-hysteresis-control-mode-only-residual-inventory-candidate-2024-01-2026-05-26"
        ),
...
def run_candidate_2024_01_control_mode_only_residual_inventory() -> dict[str, Any]:
    ...
    candidate_last_locked_size_timestamp = _load_candidate_last_locked_size_timestamp(
        subject_sources[action_2024_01.NEGATIVE_CANDIDATE.subject_id]
    )
    subject_payloads = {
    ...
    return {
        ...
        "classification_boundary": {
            "reference_surface": (
                "artifact-only inventory over the landed 2024-01 action-position-equivalence artifact, limited to rows "
                "where `switch_control_mode` is the sole surviving diff field"
            ),
            "question": (
                "Inside the exact local envelopes, after the last 2024-01 locked-size row, are control-mode-only "
...
def main() -> int:
    ...
    try:
        result = run_candidate_2024_01_control_mode_only_residual_inventory()
    except Candidate202401ControlModeResidualInventoryError as exc:
