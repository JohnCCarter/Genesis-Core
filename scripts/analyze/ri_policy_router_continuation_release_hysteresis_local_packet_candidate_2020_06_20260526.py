...
OUTPUT_FILENAME = "ri_policy_router_continuation_release_hysteresis_local_packet_candidate_2020_06_2026-05-26.json"
STATUS_OK = "continuation_release_hysteresis_local_packet_candidate_2020_06_generated"
STATUS_FAIL_CLOSED = "continuation_release_hysteresis_local_packet_candidate_2020_06_fail_closed"


class Candidate202006LocalPacketError(RuntimeError):
    pass
...
def _build_fail_closed_result(reason: str) -> dict[str, Any]:
    return {
        "audit_version": (
            "ri-policy-router-continuation-release-hysteresis-local-packet-candidate-2020-06-2026-05-26"
        ),
...
def _packet_summary(subject_payloads: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    if candidate_hits > control_hits:
        ...
        inference = (
            "The next negative-like widening candidate (`2020-06`) preserves more of the frozen triad's negative "
            "local asymmetry than the established positive control (`2023-05`) when both are rerun on the same "
            "carrier. That keeps `2020-06` alive as the next serious local-window target after `2021-04`."
        )
    elif candidate_hits == control_hits:
        status = local_packet.PACKET_STATUS_TIED
        inference = (
            "The `2020-06` candidate and the `2023-05` control hit the same number of frozen negative-rule "
            "separators, so widening beyond the exhausted `2021-04` candidate does not yet sharpen the next target "
        ...
        inference = (
            ...
            "supposed next negative-like candidate (`2020-06`), so this widening step does not sustain the current "
    ...
    return {
        ...
        "inference": inference,
        "next_hypothesis": (
            "If this chain continues, the next honest slice is a strictly local envelope around the `2020-06` "
...
def run_candidate_2020_06_local_packet() -> dict[str, Any]:
    ...
    return {
        ...
        "classification_boundary": {
            "reference_surface": (
                ...
                "negative-like candidate (`2020-06`) measured against the already characterized positive control "
                "(`2023-05`) on the frozen local negative rules"
            ),
            "question": (
                "When the next widening candidate (`2020-06`) is rerun against the established control (`2023-05`) on "
...
def main() -> int:
    ...
    try:
        result = run_candidate_2020_06_local_packet()
    except local_packet.LocalPacketError as exc:
        result = _build_fail_closed_result(str(exc))
    except Candidate202006LocalPacketError as exc:
        result = _build_fail_closed_result(str(exc))
