...
- **Required Path:** `Full`
- **Objective:** Relocate `test_dict_merge_stack_safe.py` to `tests/utils/` with no behavior change.
- **Candidate:** `tests/test_dict_merge_stack_safe.py -> tests/utils/test_dict_merge_stack_safe.py`
- **Base SHA:** `fcb3eda0`
- **Category:** `tooling`
...
- **Candidate:** `test_dict_merge_stack_safe.py`
- **Base SHA:** `fcb3eda0`

### Scope

- **Scope IN:**
  - `tests/test_dict_merge_stack_safe.py` (move source)
  - `tests/utils/test_dict_merge_stack_safe.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (batch progress row)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt15_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_dict_merge_stack_safe.py`
...
- Path-reference check i `docs/**` gav inga träffar utanför denna packet-fil; inga ytterligare icke-arkiv-referenser kräver uppdatering i BT15.
- `rg -n "tests/test_dict_merge_stack_safe.py" docs --glob "!docs/archive/**" --glob "!docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt15_2026-03-10.md"` -> inga träffar (exit code `1`, expected for no matches).
...
- `python -m pytest -q tests/utils/test_dict_merge_stack_safe.py`: **PASS** (`2 passed`)
...
- Note: first gate run surfaced `ruff` W292 (missing trailing newline) in moved test target; remediated with newline and full gate stack rerun to PASS.
- Scopediff/rename evidence (staged): `git diff --name-status --cached --find-renames` -> `A command_packet...bt15`, `M tests_subfolder_classification_plan_2026-03-09.md`, `R100 tests/test_dict_merge_stack_safe.py -> tests/utils/test_dict_merge_stack_safe.py`.
- Gate transcript context: above PASS-results are from local rerun on the staged BT15 working tree immediately before commit.

### Implementation Report (BT15)

- **Scope summary (IN):** Flytt av `tests/test_dict_merge_stack_safe.py` till `tests/utils/`, planuppdatering och BT15-packet.
...
- **Artifacts:** BT15 packet + flyttdiff + planuppdatering
