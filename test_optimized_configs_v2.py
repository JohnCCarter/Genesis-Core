...
def test_optimized_configs_v2():
    ...
    if results:
        ...
        print(f"{'Timeframe':<10} {'Return':<10} {'Win Rate':<10} {'Trades':<8} {'PF':<8} {'Sharpe':<8} {'DD':<8}")
        print("-" * 80)
        
        for result in results:
            print(f"{result['timeframe']:<10} "
        ...
        print(f"   1D: Previous 0.00% vs Optimized V2 {next((r['return'] for r in results if r['timeframe'] == '1D'), 'N/A')}%")
        print(f"   1h: Previous +0.68% vs Optimized V2 {next((r['return'] for r in results if r['timeframe'] == '1h'), 'N/A')}%")
