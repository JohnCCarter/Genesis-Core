#!/usr/bin/env python3
"""Observational selected-defensive transition-window audit for frozen RI replay evidence.

This script is RESEARCH-only and observational-only. It profiles the tiny observed
selected-defensive comparator bucket against the threshold-retained and
min-dwell-retained raw defensive-candidate buckets using a research-side analytical
mirror of the existing replay behavior. It does not modify canonical replay logic or
propose a new threshold, defensive, or dwell rule.
"""
...
AUDIT_VERSION = "scpe-ri-v1-selected-defensive-transition-window-audit-2026-04-20"
...
OUTPUT_PATH = (
    ...
    / "results"
    / "evaluation"
    / "scpe_ri_v1_selected_defensive_transition_window_audit_2026-04-20.json"
)
INPUT_PATHS = {
...
def _sort_by_recency(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return sorted(
        rows,
        key=lambda item: (
            float(item["bars_since_regime_change"]),
...
def _recency_window_summary(
    selected_rows: list[dict[str, Any]],
    threshold_rows: list[dict[str, Any]],
    min_dwell_rows: list[dict[str, Any]],
) -> dict[str, float | None]:
    selected_max = max(
        (float(row["bars_since_regime_change"]) for row in selected_rows), default=None
    )
    threshold_min = min(
        (float(row["bars_since_regime_change"]) for row in threshold_rows), default=None
    )
    min_dwell_min = min(
        (float(row["bars_since_regime_change"]) for row in min_dwell_rows), default=None
    )
    return {
        "selected_defensive_max_bars_since_regime_change": (
            _round6(selected_max) if selected_max is not None else None
        ),
        "threshold_retained_min_bars_since_regime_change": (
            _round6(threshold_min) if threshold_min is not None else None
        ),
        "min_dwell_retained_min_bars_since_regime_change": (
            _round6(min_dwell_min) if min_dwell_min is not None else None
        ),
        "threshold_recency_gap_from_selected_ceiling": (
            _round6(threshold_min - selected_max)
            if selected_max is not None and threshold_min is not None
            else None
        ),
        "min_dwell_recency_gap_from_selected_ceiling": (
            _round6(min_dwell_min - selected_max)
            if selected_max is not None and min_dwell_min is not None
            else None
        ),
    }
...
def main() -> int:
    ...
    payload = {
        ...
        "mode": "RESEARCH",
        "status": "selected-defensive-transition-window-audit-generated",
        ...
        "transition_window_summary": _recency_window_summary(
            selected_defensive_rows,
            threshold_retained_rows,
            min_dwell_retained_rows,
        ),
        "selected_defensive_examples_sorted": _sort_by_recency(selected_defensive_rows),
        "threshold_nearest_by_recency": _sort_by_recency(threshold_retained_rows)[:5],
        "min_dwell_nearest_by_recency": _sort_by_recency(min_dwell_retained_rows)[:5],
        "descriptive_guardrails": {
            ...
            "no_rule_proposal": True,
            "no_replay_quality_reinterpretation": True,
        },
        "findings": [
            "Selected-defensive rows are emitted as a descriptive comparator bucket only; the script does not classify them as a runtime-ready transition rule.",
            ...
            "The report may interpret the emitted recency-gap metrics, but the artifact remains non-authoritative and observational-only.",
        ],
    }
    ...
    if replay_root_diff:
        raise RuntimeError(
            "Baseline replay root changed during selected-defensive transition-window audit: "
            + json.dumps(replay_root_diff, ensure_ascii=False, indent=2)
        )

    print(f"[OK] Wrote selected-defensive transition-window artifact: {OUTPUT_PATH}")
    print(f"[OK] Selected-defensive rows: {len(selected_defensive_rows)}")
    print(f"[OK] Threshold-retained comparator rows: {len(threshold_retained_rows)}")
    print(f"[OK] Min-dwell-retained comparator rows: {len(min_dwell_retained_rows)}")
    return 0
