# Repo Cleanup D3L Execution Contract (2026-02-14)
...
- `scripts/diagnose_optuna_issues.py` (move)
- `scripts/archive/debug/2026-02-14/diagnose_optuna_issues.py` (new path)
...
- Move-only: inga innehållsändringar i `scripts/diagnose_optuna_issues.py`.
- Inga runtime/API/config-ändringar.
- Befintliga docs-referenser (aktiva + historiska) uppdateras inte i D3L; påverkan dokumenteras i rapporten.
...
2. D3L kontrakt + exekveringsrapport dokumenterade.
...
- D3L i detta kontrakt är kandidatvis destruktiv flytt av sista kvarvarande diagnose-script i `scripts/`.
