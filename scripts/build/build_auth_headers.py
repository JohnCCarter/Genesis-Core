...
_TARGET = (
    Path(__file__).resolve().parent.parent / "archive/2026-02/analysis/build_auth_headers.py"
).resolve()
...
def _run_deprecated_cli() -> int:
    print(
        "[DEPRECATED] scripts/build/build_auth_headers.py moved to scripts/archive/2026-02/analysis/build_auth_headers.py.",
        file=sys.stderr,
    )
    argv = sys.argv[:]
