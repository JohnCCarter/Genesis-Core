# Feature Attribution v1 — execution slice 5 packet

Date: 2026-03-31
Branch: `feature/ri-role-map-implementation-2026-03-24`
Status: `historical Feature Attribution v1 execution slice 5 packet / retained for provenance / not current branch guidance`
...
- **Category:** `obs`
- **Risk:** `HIGH` — why: this slice executes the high-sensitivity canonical strategy route to classify one admitted Feature Attribution row against the locked baseline, while remaining default-off and without broadening runner/CLI/env authority. The target seam sits inside the fib-gating route and must remain separated from the admitted `LTF override cluster`.
- **Required Path:** `Full`
- **Objective:** Execute the fifth governed Feature Attribution v1 identification slice for `HTF block seam` only and classify it descriptively as `additive`, `neutral`, `adverse`, `inconclusive`, or `invalid` against the locked baseline.
- **Candidate:** `HTF block seam`
- **Base SHA:** `68537da2`

### Scope

- **Scope IN:** this execution packet; one explicit single-row execution slice for `HTF block seam` only; locked baseline provenance from `results/research/fib_baseline_backtest_v1/summary.md` and `results/research/fib_baseline_backtest_v1/metrics.json`; existing internal execution path only (`src/core/pipeline.py`, `src/core/backtest/engine.py`, `src/core/strategy/evaluate.py`, `src/core/strategy/decision.py`) with no code changes during execution; one human-readable report under `results/research/feature_attribution_v1/reports/`; one machine-readable manifest under `results/research/feature_attribution_v1/manifests/`.
- **Scope OUT:** no source-code changes; no tests added/edited; no new CLI surface; no new env-var surface; no runner expansion; no execution of any row other than `HTF block seam`; no cluster execution; no multi-row ranking; no promotion/cutover semantics; no edits under `config/strategy/champions/**`; no fib reopening; no artifacts outside the explicitly named report/manifest outputs; no execution-path edits to `allow_ltf_override`, `ltf_override_threshold`, or `ltf_override_adaptive`.
- **Expected changed files:** `docs/decisions/feature_attribution/v1/feature_attribution_v1_exec_slice5_htf_block_identification_packet_2026-03-31.md`, `results/research/feature_attribution_v1/reports/fa_v1_htf_block_20260331_01.md`, `results/research/feature_attribution_v1/manifests/fa_v1_htf_block_20260331_01.json`
- **Max files touched:** `3`
...
| Field                   | Value                                                     |
| ----------------------- | --------------------------------------------------------- |
| `run_id`                | `fa_v1_htf_block_20260331_01`                             |
...
- execution may proceed only if prior `APPROVED_WITH_NOTES` findings for the `HTF block seam` implementation are either resolved or explicitly classified as execution-non-blocking for this slice
...
- stop if the resolved baseline effective config already has `multi_timeframe.use_htf_block = false`, because the admitted HTF block seam would then already be neutralized in the locked baseline and cannot yield a meaningful one-row candidate-vs-baseline diff
...
- `policy["feature_attribution"]["selected_row_label"] = "HTF block seam"`
...
- `python -m pytest -q tests/utils/test_decision_edge.py tests/utils/test_decision_fib_gating_contract.py tests/utils/test_decision.py`
...
- manual effective-config check that the only intentional candidate diff is neutralization of `multi_timeframe.use_htf_block`
- manual effective-config check that `allow_ltf_override`, `ltf_override_threshold`, and `ltf_override_adaptive` remain baseline-identical for the candidate
- recorded effective-config diff/hash artifact proving that the only intentional candidate delta versus baseline is `multi_timeframe.use_htf_block`, while `allow_ltf_override`, `ltf_override_threshold`, and `ltf_override_adaptive` remain baseline-identical
...
- any gate failure
- baseline effective `multi_timeframe.use_htf_block` already equals `false`, making the admitted seam already neutral in the locked baseline
- any evidence that the effective-config diff escapes `multi_timeframe.use_htf_block`
- any evidence that candidate execution altered `allow_ltf_override`, `ltf_override_threshold`, or `ltf_override_adaptive`
- any attempt to rank multiple rows or turn the descriptive label into a direct action

### Output required

- one executed single-row candidate-vs-baseline comparison for `HTF block seam`
- one descriptive classification using the locked Phase 5 vocabulary
- one report at `results/research/feature_attribution_v1/reports/fa_v1_htf_block_20260331_01.md`
- one manifest at `results/research/feature_attribution_v1/manifests/fa_v1_htf_block_20260331_01.json`
...
- `baseline_effective_use_htf_block`
- `candidate_effective_use_htf_block`
...
This packet authorizes only the fifth identification slice.
