...
- **Objective:** Continue Shard B maintainability refactor by deduplicating repeated optimizer test context setup and run invocation scaffolding while preserving assertion parity.
- **Candidate:**
  - Extract helper for repeated pair `results_root = _results_root(tmp_path)` + `run_meta_payload = _base_run_meta_payload()`.
  - Extract helper for repeated `run_optimizer(..., run_id=TEST_RUN_ID)` invocation.
- **Base SHA:** `0dc49870`
...
- **Scope IN:**
  ...
  - `docs/audit/refactor/command_packet_shard_b_refactor_2026-03-09_batch7.md`
- **Scope OUT:**
...
Current changed files are constrained to batch7 scope (`tests/test_optimizer_runner.py` + this packet).
