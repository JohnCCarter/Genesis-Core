        ...
        for symbol, timeframe in test_cases:
            try:
                ...
                if 'buy' in probas and 'sell' in probas and 'hold' in probas:
                    print(f"OK: {symbol}:{timeframe} - Buy: {probas['buy']:.3f}, Sell: {probas['sell']:.3f}, Hold: {probas['hold']:.3f}")
                    success_count += 1
                else:
        ...
        for timeframe in timeframes:
            try:
                ...
                if 'buy' in probas and 'sell' in probas:
                    print(f"OK: {symbol}:{timeframe} - Buy: {probas['buy']:.3f}, Sell: {probas['sell']:.3f}")
                    success_count += 1
                else:
                    print(f"ERROR: {symbol}:{timeframe} - Fel prediction format")
