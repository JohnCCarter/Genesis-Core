...
## 15. Uppdateringar 11 nov 2025

- `src/core/backtest/engine.py` har extra instrumentation för HTF/LTF-fibstatus samt felräkning; statistik sparas i backtestresultat under nyckeln `debug`.
- Backtest `results/backtests/tBTCUSD_1h_20251111_181048.json` visar att fib-konteksten är tillgänglig (7 586 processade barer) men 934 barer faller på `"<" not supported between instances of 'dict' and 'float'`; nytt underlag för felsökning finns i `debug["bar_errors"]`.
- Championbacktestet `results/backtests/tBTCUSD_1h_20251023_162506.json` återställdes från `_archive/20251110_131936.zip` (behåll zippen, extrahera vid behov).
- Nästa steg: identifiera var dictionaries jämförs mot floats i confidence/riskkedjan och kör backtestet igen tills `bars_processed` matchar championens 8 244/8 640.
