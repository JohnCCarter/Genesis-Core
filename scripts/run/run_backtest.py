...
Usage:
    python scripts/run/run_backtest.py --symbol tBTCUSD --timeframe 15m
    python scripts/run/run_backtest.py --symbol tETHUSD --timeframe 1h --capital 20000
"""
...
def _extract_decision_reasons(meta: dict[str, Any]) -> Any:
...
def _compose_decision_row_capture_hook(
    *,
    ...
    """Compose a passive decision-row capture hook over an optional existing hook."""
...
def _write_decision_rows(path: Path, rows: list[dict[str, Any]], fmt: str) -> Path:
    ...
    if format_name == "ndjson":
        with path.open("w", encoding="utf-8") as handle:
            for row in rows:
                ...
                handle.write("\n")
        return path
    raise ValueError(f"Unsupported decision row format: {fmt}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional output path for per-bar decision rows (captured from backtest engine hook).",
    )
    parser.add_argument(
    ...
    try:
        ...
        if args.decision_rows_out is not None:
            decision_rows_path = _write_decision_rows(
                path=args.decision_rows_out,
                rows=decision_rows,
