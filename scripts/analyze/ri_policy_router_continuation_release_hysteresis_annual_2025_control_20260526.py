...
OUTPUT_FILENAME = (
    "ri_policy_router_continuation_release_hysteresis_annual_2025_control_2026-05-26.json"
)
STATUS_OK = "continuation_release_hysteresis_annual_2025_control_generated"
STATUS_FAIL_CLOSED = "continuation_release_hysteresis_annual_2025_control_fail_closed"
DIFF_TOLERANCE = 1e-12


class Annual2025ControlError(RuntimeError):
    pass
...
def _summary_float(summary: dict[str, Any], field_name: str) -> float:
    ...
    if raw_value is None:
        raise Annual2025ControlError(f"Missing summary field {field_name!r}")
...
def _packet_summary(
    *,
    ...
    if total_return_diff > DIFF_TOLERANCE and final_capital_diff > DIFF_TOLERANCE:
        status = "annual_2025_release_zero_outperforms_baseline"
        inference = (
            "On the fixed 2025 full-year control surface, removing continuation-release hysteresis improves the top "
            "line relative to the baseline carrier. That means the seam remains directionally harmful on this broader "
            "positive-year same-stack surface, not only on one isolated monthly subject."
        )
    elif total_return_diff < -DIFF_TOLERANCE and final_capital_diff < -DIFF_TOLERANCE:
        status = "annual_2025_baseline_outperforms_release_zero"
        inference = (
            "On the fixed 2025 full-year control surface, keeping continuation-release hysteresis improves the top "
            "line relative to the release-zero variant. That means the seam contributes positively on this broader "
            "positive-year same-stack surface."
        )
    else:
        status = "annual_2025_topline_mixed_or_flat_between_baseline_and_release_zero"
        inference = (
            "On the fixed 2025 full-year control surface, the release-zero-minus-baseline top line is mixed or flat. "
            "The seam still changes many rows, but the broader annual control year cannot be reduced to a simple "
            "one-direction win from total return and final capital alone."
        )
    ...
    return {
        ...
        "inference": inference,
        "next_hypothesis": (
            "With both bad-year and positive-year annual surfaces measured, the next honest comparison is a bounded "
            "cross-year side-by-side read of where annual seam deltas stay action-neutral, versus whether one smaller "
            "row family explains any surviving annual sign difference."
        ),
    }
...
def _build_fail_closed_result(reason: str) -> dict[str, Any]:
    return {
        "audit_version": "ri-policy-router-continuation-release-hysteresis-annual-2025-control-2026-05-26",
...
def run_annual_2025_control() -> dict[str, Any]:
    ...
    all_timestamps = sorted(set(baseline_rows) | set(release_zero_rows))
    ...
    return {
        ...
        "classification_boundary": {
            "reference_surface": (
                "fixed full-year 2025 same-stack control comparison on the landed continuation_release_hysteresis "
                "carrier, holding the active router stack constant while toggling only continuation_release_hysteresis"
            ),
            "question": (
                "On the fixed 2025 annual control surface, does continuation_release_hysteresis contribute positively, "
...
def main() -> int:
    ...
    try:
        result = run_annual_2025_control()
