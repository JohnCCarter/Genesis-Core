...
- `config/optimizer/3h/phased_v3/tBTCUSD_3h_phased_v3_phaseD.yaml` — Phase D (75 trials, 7 risk_state tunable + alla A/B fixerade)
- `config/optimizer/3h/phased_v3/tBTCUSD_3h_phased_v3_phaseE_oos.yaml` — Phase E OOS (alla fixerade, 2025)
- `config/optimizer/3h/phased_v3/best_trials/` — best_trial JSON för varje fas
...
PYTHONIOENCODING=utf-8 TQDM_DISABLE=1 PYTHONPATH="$(pwd)/src" python -m core.optimizer.runner config/optimizer/3h/phased_v3/tBTCUSD_3h_phased_v3_phaseE_oos.yaml
```
