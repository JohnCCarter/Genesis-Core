#!/usr/bin/env python3
"""Counterfactual no-trade release revision over frozen RI replay evidence.

This script is RESEARCH-only. It does not modify the canonical replay root and does not
change runtime code. It evaluates a selective, fail-closed early-release exception for a
...
REVISION_VERSION = "scpe-ri-v1-no-trade-release-revision-2026-04-20"
...
def _build_findings(
    changed_count: int, blocked_delta: int, no_trade_delta: float, continuation_trade_delta: int
) -> list[str]:
    findings = [
        f"Selective early-release changed exactly {changed_count} baseline-blocked no-trade continuation candidates and no rows outside the allowed frozen subset.",
        f"Blocked switch count moved by {blocked_delta} relative to baseline, while the canonical replay root remained unchanged.",
        f"No-trade rate moved by {no_trade_delta:+.6f} under the local counterfactual override.",
        f"Continuation-policy trade count moved by {continuation_trade_delta} under the counterfactual replay-quality view.",
    ]
    return findings
...
def main() -> int:
    ...
    payload = {
        ...
        "mode": "RESEARCH",
        "status": "no-trade-release-revision-generated",
