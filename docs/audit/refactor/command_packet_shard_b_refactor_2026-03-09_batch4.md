...
- **Objective:** Continue Shard B maintainability refactor by deduplicating repeated trial-setup and backtest-payload test scaffolding in optimizer runner tests while preserving assertion parity.
- **Candidate:**
  - Extract shared `TrialConfig` helper for repeated `snapshot/symbol/timeframe/dates` construction in multiple tests.
  - Extract shared backtest-result payload helper used by two `fake_run_backtest_direct` stubs.
- **Base SHA:** `475204ed`
...
- **Scope IN:**
  ...
  - `docs/audit/refactor/command_packet_shard_b_refactor_2026-03-09_batch4.md`
- **Scope OUT:**
...
Current changed files are constrained to batch4 scope (`tests/test_optimizer_runner.py` + this packet).
Remediation note: out-of-scope `scripts/build/__pycache__/` artifact was removed and gate stack re-run after cleanup.
