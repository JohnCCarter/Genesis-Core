...
ARTIFACT_DIR = (
    ROOT_DIR / "results" / "backtests" / "ri_policy_router_continuation_release_hysteresis_20260504"
)
SUMMARY_PATH = ARTIFACT_DIR / "continuation_release_hysteresis_summary.json"
ROW_DIFFS_PATH = ARTIFACT_DIR / "continuation_release_hysteresis_row_diffs.json"
...
def _representative_release_rows(
    baseline_rows: dict[str, dict[str, Any]],
    release_zero_rows: dict[str, dict[str, Any]],
) -> list[dict[str, Any]]:
    timestamps = sorted(set(baseline_rows) | set(release_zero_rows))
    output: list[dict[str, Any]] = []
    for ts in timestamps[:REPRESENTATIVE_LIMIT]:
        output.append(
            {
                "timestamp": ts,
                "baseline": baseline_rows.get(ts),
                "release_zero": release_zero_rows.get(ts),
                "row_differs": baseline_rows.get(ts) != release_zero_rows.get(ts),
            }
        )
    return output
...
def main() -> int:
    ...
    summary_payload = {
        ...
        "comparison": {
            ...
            "representative_diffs": diffs[:REPRESENTATIVE_LIMIT],
            "representative_behavioral_diffs": [diff for diff in diffs if diff["behavior_changed"]][
                :REPRESENTATIVE_LIMIT
            ],
            "representative_continuation_release_rows": _representative_release_rows(
