...
OUTPUT_FILENAME = "ri_policy_router_insufficient_evidence_truth_surface_correction_2026-04-30.json"
SUBJECT_SYMBOL = "tBTCUSD"
SUBJECT_TIMEFRAME = "3h"
CORRECTED_TARGET_YEAR = "2024"
CONTROL_YEAR = "2020"
PACKET_REFERENCE = Path(
    "docs/decisions/regime_intelligence/policy_router/"
    "ri_policy_router_insufficient_evidence_truth_surface_correction_precode_packet_2026-04-30.md"
)
BASELINE_REFERENCE = Path(
    "docs/analysis/regime_intelligence/policy_router/"
    "ri_policy_router_insufficient_evidence_counterfactual_screen_2026-04-30.md"
)
CORRECTED_TARGET_TIMESTAMPS = (
    "2024-07-13T09:00:00+00:00",
    "2024-07-14T09:00:00+00:00",
    "2024-07-14T18:00:00+00:00",
)
CORRECTED_COMPARISON_TIMESTAMPS = (
    "2024-07-12T09:00:00+00:00",
    "2024-07-12T18:00:00+00:00",
)
CORRECTED_CONTEXT_TIMESTAMPS = (
...
EXPECTED_2024_ENVELOPE_COUNT = 7
CANDIDATE_FIELDS = (
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize one bounded truth-surface correction retest on the exact July 2024 "
            "negative-year insufficient-evidence subset versus the frozen exact 2020 "
            "positive-year weak control cluster."
        )
    )
    parser.add_argument(
        "--base-sha",
...
def _assert_exact_2024_envelope(
    rows: list[Any],
) -> dict[str, Any]:
    target_ts = _normalized_constant_timestamps(CORRECTED_TARGET_TIMESTAMPS)
    ...
    if len(local_rows) != EXPECTED_2024_ENVELOPE_COUNT:
        raise LocalWindowEvidenceError(
            "July 2024 local envelope count mismatch: "
            f"expected {EXPECTED_2024_ENVELOPE_COUNT}, got {len(local_rows)}"
        )

    expected_all = {
        *[ts.isoformat() for ts in _normalized_constant_timestamps(CORRECTED_TARGET_TIMESTAMPS)],
        *[
            ts.isoformat()
            for ts in _normalized_constant_timestamps(CORRECTED_COMPARISON_TIMESTAMPS)
        ],
        *[ts.isoformat() for ts in _normalized_constant_timestamps(CORRECTED_CONTEXT_TIMESTAMPS)],
    }
...
def load_corrected_2024_cohorts() -> tuple[dict[str, list[dict[str, Any]]], dict[str, Any]]:
    rows, candles, timestamp_to_index = _load_year_context(CORRECTED_TARGET_YEAR)
    envelope_metadata = _assert_exact_2024_envelope(rows)

    target_rows = select_exact_rows(
        rows,
        exact_timestamps=_normalized_constant_timestamps(CORRECTED_TARGET_TIMESTAMPS),
        expected_reason="insufficient_evidence",
        expected_action_pair=("LONG", "NONE"),
    )
    comparison_rows = select_exact_rows(
        rows,
        exact_timestamps=_normalized_constant_timestamps(CORRECTED_COMPARISON_TIMESTAMPS),
        expected_reason="stable_continuation_state",
        expected_action_pair=("NONE", "LONG"),
    )
    context_rows = select_exact_rows(
        rows,
        exact_timestamps=_normalized_constant_timestamps(CORRECTED_CONTEXT_TIMESTAMPS),
        expected_reason="stable_continuation_state",
        expected_action_pair=("LONG", "NONE"),
    )

    cohorts = {
        "corrected_2024_target": _serialize_rows(
            target_rows,
            cohort_name="corrected_2024_target",
            year=CORRECTED_TARGET_YEAR,
            candles=candles,
            timestamp_to_index=timestamp_to_index,
        ),
        "corrected_2024_nearby_displacement": _serialize_rows(
            comparison_rows,
            cohort_name="corrected_2024_nearby_displacement",
            year=CORRECTED_TARGET_YEAR,
            candles=candles,
            timestamp_to_index=timestamp_to_index,
        ),
        "corrected_2024_nearby_blocked_context": _serialize_rows(
            context_rows,
            cohort_name="corrected_2024_nearby_blocked_context",
            year=CORRECTED_TARGET_YEAR,
            candles=candles,
            timestamp_to_index=timestamp_to_index,
        ),
...
def _balanced_accuracy(
    ...
    if target_selection_rate is None or control_selection_rate is None:
        return None
    return _round_or_none((target_selection_rate + (1.0 - control_selection_rate)) / 2.0)


def _truth_surface_check(cohort_summaries: dict[str, dict[str, Any]]) -> dict[str, Any]:
    target_mean = cohort_summaries["corrected_2024_target"]["metric_summary"][
        "fwd_16_close_return_pct"
    ]["mean"]
    ...
    return {
        "corrected_2024_target_fwd16_mean": target_mean,
        "control_2020_target_fwd16_mean": control_mean,
        "corrected_2024_target_positive_on_fwd16_mean": target_positive,
        "control_2020_target_negative_on_fwd16_mean": control_negative,
        "corrected_2024_target_exceeds_control_2020_on_fwd16_mean": target_exceeds_control,
...
def _evaluate_rules(
    ...
    *,
    ...
    for rule_group in rules:
        ...
        target_rate = selection_by_cohort["corrected_2024_target"]["selection_rate"]
        control_rate = selection_by_cohort["control_2020_target"]["selection_rate"]
        survives = bool(
            target_rate is not None
            and control_rate is not None
            and float(target_rate) >= SURVIVAL_MIN_2024_SELECTION_RATE
            and float(control_rate) <= SURVIVAL_MAX_2020_SELECTION_RATE
        )
        evaluations.append(
            {
                "stage": stage,
                ...
                "corrected_target_selection_rate": target_rate,
                "control_target_selection_rate": control_rate,
                "balanced_accuracy": _balanced_accuracy(target_rate, control_rate),
                "survives": survives,
            }
        )
    ...
    return sorted(
        ...
        key=lambda item: (
            ...
            -float(item["corrected_target_selection_rate"] or 0.0),
            item["rule_count"],
...
def run_truth_surface_correction(base_sha: str) -> dict[str, Any]:
    corrected_2024_cohorts, corrected_envelope = load_corrected_2024_cohorts()
    control_cohorts = load_control_2020_cohorts()
    cohorts = {**corrected_2024_cohorts, **control_cohorts}
    ...
    target_rows = cohorts["corrected_2024_target"] + cohorts["control_2020_target"]
    ...
    return {
        "audit_version": "ri-policy-router-insufficient-evidence-truth-surface-correction-2026-04-30",
        "base_sha": base_sha,
        "status": result_state,
        ...
        "subject": {
            ...
            "corrected_target_year": CORRECTED_TARGET_YEAR,
        ...
        "fail_closed_contract": {
            "statement": (
                "2024 and 2020 inputs are restricted to the exact packet timestamps on the annual "
                "action-diff surface plus curated candles for observational metrics only, the July "
                "2024 envelope must remain exactly 7 rows with no unlabeled extras, and the helper "
                "exits nonzero on any row-lock, count, year, zone, action-pair, or truth-surface mismatch."
            )
        },
        "candidate_screen_contract": {
            ...
            "truth_surface_prerequisite": (
                "The fixed July 2024 target side must remain positive on mean fwd_16, the fixed 2020 "
                "control side must remain negative on mean fwd_16, and the July 2024 target mean must "
                "exceed the 2020 control mean; otherwise no mechanical separator is promoted."
            ),
            "survival_rule": {
                "min_corrected_2024_target_selection_rate": SURVIVAL_MIN_2024_SELECTION_RATE,
                "max_control_2020_target_selection_rate": SURVIVAL_MAX_2020_SELECTION_RATE,
            },
        },
        "inputs": {
            "action_diff_root": str(ACTION_DIFF_ROOT_RELATIVE),
            "corrected_2024_diff": str(
                ACTION_DIFF_ROOT_RELATIVE
                / f"{CORRECTED_TARGET_YEAR}_enabled_vs_absent_action_diffs.json"
            ),
        ...
        "artifact_row_lock": {
            "corrected_2024_target_timestamps": [
                _normalize_timestamp(value).isoformat() for value in CORRECTED_TARGET_TIMESTAMPS
            ],
            "corrected_2024_nearby_displacement_timestamps": [
                _normalize_timestamp(value).isoformat() for value in CORRECTED_COMPARISON_TIMESTAMPS
            ],
            "corrected_2024_nearby_context_timestamps": [
                _normalize_timestamp(value).isoformat() for value in CORRECTED_CONTEXT_TIMESTAMPS
            ],
            ...
            "corrected_2024_envelope": corrected_envelope,
        },
        "cohorts": cohort_summaries,
        "truth_surface_check": truth_surface,
        "descriptive_comparison": {
            "corrected_target_vs_2024_nearby_displacement": _descriptive_gap(
                cohort_summaries["corrected_2024_target"],
                cohort_summaries["corrected_2024_nearby_displacement"],
                "fwd_16_close_return_pct",
            ),
            "corrected_target_vs_2024_nearby_blocked_context": _descriptive_gap(
                cohort_summaries["corrected_2024_target"],
                cohort_summaries["corrected_2024_nearby_blocked_context"],
                "fwd_16_close_return_pct",
            ),
            "corrected_target_vs_2020_control_target": _descriptive_gap(
                cohort_summaries["corrected_2024_target"],
...
def main() -> int:
    ...
    result = run_truth_surface_correction(base_sha=args.base_sha)
