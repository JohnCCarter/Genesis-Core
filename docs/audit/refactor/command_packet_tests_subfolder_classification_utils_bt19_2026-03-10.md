...
- **Risk:** `LOW` — why: single-candidate test relocation av utility-lik test utan aktiva docs-referenser.
- **Required Path:** `Full`
- **Objective:** Relocate `test_symbols.py` to `tests/utils/` with no behavior change.
- **Candidate:** `tests/test_symbols.py -> tests/utils/test_symbols.py`
- **Base SHA:** `abbd4a8e`
- **Category:** `tooling`
...
- **Candidate:** `test_symbols.py`
- **Base SHA:** `abbd4a8e`

### Scope

- **Scope IN:**
  - `tests/test_symbols.py` (move source)
  - `tests/utils/test_symbols.py` (move target)
  - `docs/audit/refactor/tests_subfolder_classification_plan_2026-03-09.md` (batch progress row)
  - `docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt19_2026-03-10.md`
- **Scope OUT:**
...
- `python -m ruff check .`
- `python -m pytest -q tests/utils/test_symbols.py`
...
- Path-reference check i `docs/**` gav inga träffar utanför denna packet-fil; inga ytterligare icke-arkiv-referenser kräver uppdatering i BT19.
- `rg -n "tests/test_symbols.py" docs --glob "!docs/archive/**" --glob "!docs/audit/refactor/command_packet_tests_subfolder_classification_utils_bt19_2026-03-10.md"` -> inga träffar (exit code `1`, expected for no matches).

### Review + verification evidence

- Opus pre-code review: **APPROVED_WITH_NOTES** -> note remediated (skill exit-code claim clarified) -> **GO**.
- `python -m pre_commit run --all-files`: **PASS**
- `python -m ruff check .`: **PASS**
- `python -m pytest -q tests/utils/test_symbols.py`: **PASS** (`4 passed`)
...
### Implementation Report (BT19)

- **Scope summary (IN):** Flytt av `tests/test_symbols.py` till `tests/utils/`, planuppdatering och BT19-packet.
...
- **Artifacts:** BT19 packet + flyttdiff + planuppdatering
